...
\reporttitle{Stock assessment of bigeye tuna in the western and central Pacific Ocean: 2026}
\reportnumber{WCPFC-SC22-2026/SA-WP-06 (Rev.01)}
\reporttype{SC22}
...
\urlstyle{same}
\hypersetup{
  pdftitle={Stock assessment of bigeye tuna in the western and central Pacific Ocean: 2026},
...
\item
  Release-group-specific tag mixing periods based on an individual-based
  tag-mixing model (consistent with the yellowfin tuna peer-review
  recommendation in Punt et al. (2023)), and removal of Coral Sea
  tagging data (supported by PAW)
\item
...
Across the 80 retained models, the natural-mortality scale and
...
a strong response to tag-data overdispersion for \(\tau\) values from
1.0 to approximately 1.4, above which estimates of depletion and
population scale changed little. The global-CPUE period used to inform
the regional-scaling penalty was another influential assumption.
...
\begin{itemize}
\item
  Median recent depletion (2021--2024; \(SB_{recent}\)/\(SB_{F=0}\)) is
  estimated at 0.277 (central 80\% interval including structural and
  available estimation uncertainty: 0.172--0.395; central 95\% interval:
  0.136--0.442).
\item
...
\item
  ...
  depletion ratio at or above the average \(SB/SB_{F=0}\) for
  2012--2015. Across the 80 ensemble models, excluding estimation
  uncertainty, this average was 28.8\%. The corresponding average for
  2021--2024 was 27.8\%, giving a recent-to-2012--2015 spawning
  depletion ratio of 96.7\%; stock status was therefore slightly below
  the objective (Table~\ref{tbl-cmm-depletion}).
\item
  Median recent fishing mortality (2020--2023),
  \(F_{recent}\)/\(F_{MSY}\), was 0.838 (central 80\% interval including
  structural and available estimation uncertainty: 0.592--1.206; central
  95\% interval: 0.460--1.351).
\item
  Although the stock is assessed as more depleted and subject to higher
  fishing mortality than in 2023, the ensemble indicates that it is not
  overfished under the adopted LRP criterion (18.6\% probability below
  the LRP) and is not undergoing overfishing based on the median
  \(F_{recent}\)/\(F_{MSY}<1\).
\item
...
\subsection{Open and reproducible assessment
resources}\label{open-and-reproducible-assessment-resources}

The assessment results and supporting analyses can be opened directly:
...
\item
  ...
  report}.
\item
  \textbf{Model development and fit:}
  \href{https://pacificcommunity.github.io/ofp-sam-bet-2026-stepwise/interactive-model-viewer.html}{stepwise
  viewer},
  \href{https://pacificcommunity.github.io/ofp-sam-bet-2026-diagnostic/bet-2026-diagnostic-report.html}{diagnostic
  report} and
  \href{https://pacificcommunity.github.io/ofp-sam-bet-2026-diagnostic/bet-2026-likelihood-profile-viewer.html}{likelihood-profile
  viewer}.
\item
  \textbf{Model diagnostics and sensitivities:}
  ...
  and
  \href{https://pacificcommunity.github.io/ofp-sam-bet-2026-sensitivity/bet-2026-sensitivity-interactive-viewer.html}{one-off
  sensitivity viewer}.
\item
  \textbf{Management uncertainty and projections:}
  \href{https://pacificcommunity.github.io/ofp-sam-bet-2026-ensemble/bet-2026-ensemble-interactive-viewer.html}{ensemble
  viewer}.
\end{itemize}

The corresponding repositories and pinned build environment are
documented once in Table~\ref{tbl-open-science-resources}. These linked,
versioned resources expose detail that cannot be shown legibly in static
figures and support transparent, reproducible exploration beyond that
available for previous WCPO tuna assessments.
...
\begin{itemize}
\item
  Simplified spatial structure from nine to five regions and revised
  fishery definitions (Figure~\ref{fig-map-5regions})
\item
...
nautical miles) Figure~\ref{fig-tag-displacement-map}. These occasional
...
catch, effort, length-frequency data for the fisheries defined in the
analysis, and tag-recapture data. The fitted length compositions combine
observed lengths with longline weight observations converted externally
to length. Conditional age-at-length data are also used directly as data
...
papers highlighted at the end of Section~\ref{sec-introduction} for
...
assessment Figure~\ref{fig-map-5regions}. The PAW also recommended to
...
The correspondence between the former nine-region structure and the five
regions used in 2026 is shown explicitly in
Figure~\ref{fig-map-spatial-comparison}.
...
33 in the current assessment (Table~\ref{tbl-fisheries-definitions}).
...
is applied only to that time period Section~\ref{sec-catch-effort-LL}.
...
Table~\ref{tbl-fisheries-definitions}. The length-composition data for
the extraction fisheries are assumed to represent the actual composition
...
in Table~\ref{tbl-fisheries-definitions}. Catches by the longline
...
Figure~\ref{fig-catch-hist-full} and a regional breakdown is provided in
Figure~\ref{fig-catch-hist-regional}. Catches by fishery groups are
provided in Figure~\ref{fig-catch-by-fishery-longline},
Figure~\ref{fig-catch-by-fishery-ps} and
...
\item
  The notable increase in handline catch reported for 2024. This
  resulted from a change in how Indonesia provided its 2026 catch
  statistics and allocated bigeye catch between small- and large-fish
  handline fisheries. Indonesia determined that a large portion of the
  catch previously reported as smaller-fish handline gear (part of the
  `other' category) should be allocated to the larger-fish handline
  category (see Vidal et al. (2026)). A historical adjustment has not
  yet been made and is a priority under the WPEA project. The
  terminal-year increase is expected to have only a minor influence on
  the current assessment, but the historical series should be corrected
...
Figure~\ref{fig-data-coverage-tile} provides details of the temporal
...
explained in Section~\ref{sec-sizefreq-likelihood}. Summaries of the
...
improve the reliability and representativeness of size compositions.
This includes aggregating data by model region and fishery, filtering
unreliable sources and small samples, and reweighting size frequencies
to represent catches in extraction fisheries and relative abundance in
index fisheries. Only length data are available for purse-seine,
pole-and-line and other miscellaneous fisheries in the
Indonesia--Philippines--Vietnam region; both weight and length
observations are available for longline fisheries. For the 2026
assessment, following earlier recommendations (Day et al., 2023; Teears
et al., 2023) and PAW discussion (Hamer, 2026), longline weight
observations were converted externally to length and combined with
observed length data (Peatman et al., 2026b). The final assessment
...
preparation, conversion, filtering and reweighting process is described
by Peatman et al. (2026b).
...
Figure~\ref{fig-tag-release-recapt} and
...
data are not very informative about steepness (ISSF, 2011); hence, it
was fixed at 0.90 in the diagnostic model and sensitivity analyses used
values of 0.65, 0.80 and 0.95. The high CV (2.2) of the annual
log-recruitment deviations ensured that the SRR had negligible influence
on recruitment and other model estimates, as recommended by Ianelli et
al. (2012).
...
prior. Age-specific deviations can also be estimated or entered as a
fixed \(M\)-at-age function. The latter approach was taken in the 2020
bigeye and yellowfin assessments (Ducharme-Barth et al., 2020a; Vincent
et al., 2020), using an approach applied to other tunas in the WCPO and
EPO (Harley \& Maunder, 2003; S. D. Hoyle, 2008; Hoyle \& Nicol, 2008).
That external function depends on empirical and assumed inputs for sex
...
yellowfin peer review and the CAPAM Tuna Stock Assessment Good Practices
Workshop, the 2023 bigeye assessment instead estimated a Lorenzen
\(M\)-at-age function internally (Day et al., 2023). However, that model
was not stable to jittering, and SC19 raised concern that jointly
estimating growth and \(M\) may contribute to instability. The 2026 PAW
therefore recommended fixing the adult/asymptotic \(M\) scale, based on
assumed \(M\) at age 40 quarters, initially at the 2023 diagnostic-model
estimate, while retaining the inverse relationship between the Lorenzen
shape and the internally estimated growth curve (Hamer, 2026).
...
(Figure~\ref{fig-maturity-ogive}). Another added benefit is that this
...
Figure~\ref{fig-select-age} and their equivalent length-based curves are
shown in Figure~\ref{fig-select-length}. Selectivity was modelled using
cubic splines, which provide more flexibility than a fixed functional
relationship with age and require fewer parameters than separate
age-specific estimates. The number of spline nodes was chosen to
...
Selectivity was time-invariant and fishery-specific in this assessment.
...
in Table~\ref{tbl-tag-definitions}. Previous assessments have assumed
...
bias estimation of management quantities (Kolody \& Hoyle, 2015). Mixing
...
Previous bigeye assessments applied fixed one- or two-quarter mixing
periods to all tag-release groups, based on qualitative comparisons of
post-release recaptures and purse-seine catch maps (Teears et al.,
2023). The 2023 yellowfin peer review noted the individual-based
tag-mixing approach developed for skipjack (Castillo Jordan et al.,
2022; Scutt Phillips et al., 2022). It simulates dispersal and exposure
to fishing mortality for each release group and a contemporaneous
untagged population, accounting for the location, timing, environment
and fishing context of each release event. The Ikamoana Lagrangian model
tracks individual particles using forcings and parameters from SEAPODYM
(Scutt Phillips et al., 2018; Senina et al., 2020). The peer-review
panel recommended exploring this approach for yellowfin and bigeye, and
release-group-specific mixing periods were subsequently estimated for
this assessment (Scutt Phillips et al., 2026).
...
In the 2023 assessment, size-frequency compositions were fitted with a
robust-normal likelihood, with nominal sample sizes divided by 10, 20
(diagnostic model) or 40 to allow for overdispersion. A
...
so it was not retained (Thorson et al., 2017).

The 2026 diagnostic model used the DM likelihood without random effects
...
the incident effective sample size as
...
evaluated values including 10, 25, 40, 100 and the MFCL default of
...
compositions (Francis, 2011). We therefore set \(N_{\max}=25\),
...
The final controls are summarised in Table~\ref{tbl-dm-configuration}.
The common log-concentration intercept (\inlinecode{fish_pars(22)}) had
...
The relative-sample-size exponent (\inlinecode{fish_pars(23)}) was
...
As discussed in Section~\ref{sec-likelihood-profile}, the tag data was
...
\item
  ...
  preferably zero.
\item
  5. The number and composition of pairs of highly correlated estimated
  parameters (\(|r| > 0.90\)).
\end{enumerate}
...
\textbf{Jitter analysis} Thirty phase-aware jitter fits were used to
...
ordinary \inlinecode{makepar}-generated fitting path, used the completed
diagnostic fit to identify which parameters were estimated and in which
phase they became active, and then repeated the full fitting schedule.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Thirty runs used a perturbation CV of 0.1 across the 12 estimation
  phases. Only parameters estimated in the completed diagnostic model
  were included.
\item
  After Phase 1, parameters already active were perturbed; parameters
  ...
  phase-specific initialisation used in the original fit.
\item
...
\item
  ...
  objective values and derived quantities were compared with the
  unperturbed diagnostic fit.
\end{enumerate}

Results for Jitter models are shown in Section~\ref{sec-diag-jitter}.

\textbf{Self tests} This assessment included the first formal simulation
self-test in the WCPO tuna assessment series, following the general
...
diagnostic model was treated as the operating model and 50 independent
...
\inlinecode{doitall} schedule, and all 50 refits met the archived MGC
criterion of \(10^{-4}\). Relative recovery error was calculated against
the generating quantity for each replicate. These empirical
distributions test internal consistency and practical estimability under
the configured observation models; they are not parameter confidence
intervals and do not determine whether the diagnostic model is an
unbiased representation of the unknown population. Results are presented
in Section~\ref{sec-selftest}.

\textbf{ASPM} A strict age-structured production model (ASPM) diagnostic
(Carvalho et al., 2017; Carvalho et al., 2021; Maunder \& Piner, 2015;
Minte-Vera et al., 2021) was fitted by conditioning on the observed
catches and retaining only the five CPUE indices in the observation
likelihood. Tagging, length- and weight-frequency, catch-at-age,
conditional age-at-length and penalty components were excluded. Biology,
movement, selectivity, regional recruitment allocation and the initial
age--region structure were fixed at their diagnostic-model values.
Temporal relative recruitment was set to one, regional recruitment
deviations were set to zero and the stock--recruit relationship was
inactive; the only estimated parameter family was mean recruitment,
which determines the overall population scale. Comparison with the
integrated model therefore shows the biomass scale and trend supported
by the catch-conditioned CPUE signal under constant recruitment
...
presented in Section~\ref{sec-retro-analysis}. The Mohn's rho statistic
...
presented for the diagnostic model Section~\ref{sec-likelihood-profile}.
...
For management reporting, annual dynamic depletion compares spawning
biomass in year \(y\) with mean unfished spawning biomass over the
preceding ten years, excluding year \(y\).

Two related but distinct depletion quantities were used for management
reporting. The WCPFC-adopted limit reference point (LRP) for bigeye is
\(SB_{recent}/SB_{F=0}=0.20\), where the numerator is mean spawning
biomass during 2021--2024 and the denominator is mean unfished spawning
biomass during 2014--2023. CMM 2025-02 paragraph 10 separately states
that, pending agreement on a target reference point, the
spawning-biomass depletion ratio is to be maintained at or above its
2012--2015 average. This comparison was calculated within each model as

\[
D_y=\frac{SB_y}{\frac{1}{10}\sum_{j=y-10}^{y-1}SB_{j,F=0}},\qquad
D_{recent}=\frac{1}{4}\sum_{y=2021}^{2024}D_y,\qquad
D_{2012\text{--}2015}=\frac{1}{4}\sum_{y=2012}^{2015}D_y,
\]

for each of the 80 central models, excluding estimation uncertainty. The
headline comparison is the arithmetic mean of \(D_{recent}\) across the
equal-weight models divided by the corresponding arithmetic mean of
\(D_{2012\text{--}2015}\). This recent-to-2012--2015 spawning depletion
ratio is a ratio of two depletion quantities, not a probability, and is
distinct from the LRP quantity formed from separate fished and unfished
period means. The comparison with the CMM 2025-02 objective is reported
in Table~\ref{tbl-cmm-depletion}, with the other management and
reference-point quantities in Table~\ref{tbl-manage-quant} and
Table~\ref{tbl-ref-points}.
...
as the mean over the recent period 2020--2023, consistent with the
definition of \(F_{recent}\) used in this assessment. We do not include
...
For the standard yield analysis (Section~\ref{sec-method-yield}), the
...
Thirty-year stochastic projections were conducted within MFCL for
2025--2054, following the general framework in Pilling et al. (2016).
Each of the 80 retained assessment models was paired with ten future
...
fishery and quarter at their observed 2022--2024 mean catches; missing
...
Results are reported in Section~\ref{sec-projection-results}.
...
the 2026 diagnostic model is described in Section~\ref{sec-stepwise} and
displayed in Figure~\ref{fig-stepwise-panel} and
Figure~\ref{fig-stepwise-panel-reduce}. Each step below is compared with
...
tuning process. The diagnostic model represents the culmination of a
number of steps that incorporated changes from the previous assessment,
recommendations from the Pre-assessment Workshop (PAW) and Scientific
Committee, new data and biological information, and methodological
developments. It provides a fully examined reference model for
sensitivity analyses and construction of the uncertainty ensemble;
management advice is based on that ensemble rather than on the
diagnostic model alone.
...
\item
  ...
  selectivity plus weak F10 non-decreasing penalty
\item
  \textbf{{[}16{]}} Implement release-group-specific mixing periods,
  K-statistic cut-off 0.2: \textbf{fifth clear but not major change},
  with lower \(SB_{F=0}\) and \(SB\), and greater depletion.
\item
...
\item
  ...
  \(SB\)/\(SB_{F=0}\) and \(SB\), with higher early \(SB\) and
  \(SB_{F=0}\) and a steeper decline in \(SB\).
\item
...
Overall, only minor to moderate changes in terminal estimates, relative
to the preceding step, were observed when updating the data to 2024
(less depleted and higher spawning potential), implementing the new CPUE
approach (more depleted and lower spawning potential), implementing
release-group-specific mixing periods (moderately more depleted and
slightly lower spawning potential), applying effort creep (slightly less
depleted and higher spawning potential), and implementing the
Dirichlet-multinomial likelihood (slightly more depleted and slightly
lower spawning potential). The increased initial biomass, steeper
decline and slightly higher terminal biomass after applying effort creep
were counter-intuitive. Each effort-creep scenario nevertheless
represents a separate set of CPUE indices to which the model is fitted,
so the starting biomass can shift. The alternatives were therefore
treated as independent hypotheses, although methods that separate
...
(negative binomial) (step 20) Figure~\ref{fig-stepwise-panel-reduce}.
...
The model reproduced the main trends in the standardised CPUE indices
for all five regions (Figure~\ref{fig-cpue-fit-residuals}). The fit was
weaker in Region 1 during approximately the final 15 years, when
...
Fishery-level fits, conditional predictive intervals and the
...
\textbf{Longline fisheries:} Fits for most longline extraction fisheries
(fisheries 1--11) and longline index fisheries (fisheries 29--33) were
acceptable. The Region 5 index fishery had particularly poor size-data
coverage, and the model overestimated larger length classes while
underestimating small-to-medium length classes. The LL.ALL extraction
fishery in Region 5 had more data and a better fit.
...
intervals across time (Figure~\ref{fig-length-fit-timeseries-ll}). The
...
\textbf{Other fisheries:} Length-composition fits were also relatively
good for most non-longline and purse-seine fisheries. Time-series fits
to mean length were clearest for fisheries with more observations, such
as the Philippines purse-seine fishery in Region 2 and the important
associated purse-seine fishery in Region 3, which accounts for a large
amount of the bigeye catch. The model predicted the mean-length dynamics
of this fishery relatively well
(Figure~\ref{fig-length-fit-timeseries-other}).

\subsubsection{Tagging data}\label{sec-fit-tagging}

The programme-specific and aggregate tag-attrition predictions fitted
the broad pattern of observed recaptures relatively well
(Figure~\ref{fig-tag-attrition-programmes}), although the aggregate
prediction overestimated some recaptures between 10 and 15 quarters at
liberty. Tag-attrition predictions were closer to the observations when
\(\tau=1\) (not shown), consistent with the greater likelihood weight
assigned to tag data in that model.

There was notable variation in recaptures among tag-recapture groups and
through time (Figure~\ref{fig-tag-recapture-fit-years-groups}). Eight of
the 16 plotted groups dominated the recaptures, and the diagnostic-model
predictions generally matched the observations for those dominant
groups.

\subsubsection{Tag reporting rates}\label{sec-tag-reporting-rates}

The 12 estimated MFCL reporting-rate groups are shown together in
...
recapture-fishery groups and programmes. The more informative RTTP and
PTTP penalties based on tag-seeding data constrained groups 7, 10, 14,
17 and 18, although groups 17 and 18 favoured higher reporting rates.
Group 23 (JPTP, fisheries 12--13) reached the 0.99 upper bound because
...
estimated nor plotted.

\subsubsection{Conditional age-at-length}\label{sec-fit-CAAL}

The estimated growth curve for the diagnostic model, which is informed
by both the CAAL and size-composition data, is shown in
Figure~\ref{fig-growth-curve}. Regional fits to the
conditional-age-at-length (CAAL) data are shown in
Figure~\ref{fig-caal-fits-regions}. For length bins from around 70 to
120 cm, which contain most observations, predicted median ages generally
tracked the data well. At smaller sizes the model tended to slightly
overpredict median age, whereas median age was generally underpredicted
from around 120 to 150 cm; predictions were closer to the observations
at the largest sizes.

At the model-region level, Region 1 had a reasonable amount of data
after the addition of samples from north of \(10^{\circ}\,\mathrm{N}\)
and showed the best fit, with model estimates tracking observed mean age
at length well. Regions 2 and 4 had relatively few observations; the fit
was weaker in Region 2 but reasonable in Region 4. Regions 3 and 5 had
more data and, as in the aggregate data, predictions were reasonable to
approximately 120 cm, above which mean age at length tended to be
underpredicted (Figure~\ref{fig-caal-fits-regions}).
...
The diagnostic model reached a maximum gradient component of
...
The inverse-Hessian correlation matrix contained 22 parameter pairs with
...
recruitment-distribution deviations.

\subsubsection{Jitter}\label{sec-diag-jitter}

The phase-aware jitter analysis explored a broader set of initial values
than the final-MLE jitter used in earlier assessments. All 30 fits
...
fit. The lowest objective was 89,469.7, which was 1,345.2 units below
the diagnostic objective of 90,814.9
(Figure~\ref{fig-jitter-scatter-plot}). This indicates local structure
in the likelihood surface. Recruitment and some regional trajectories
varied among retained solutions, whereas terminal depletion and other
management quantities were comparatively stable
(Figure~\ref{fig-jitter-derived-values-diag-panel} and
Figure~\ref{fig-jitter-regional-depletion}). The result is therefore a
...
All 50 pseudo-data refits completed and met the recorded convergence
criterion, with maximum gradient components from \(5.13\times10^{-5}\)
to \(9.97\times10^{-5}\). Median whole-series pseudo-data errors ranged
from -0.23\% to 0.07\%, and the generating trajectory was inside the
pointwise central 95\% empirical refit range at 385 of 438 annual
quantity--year points (87.9\%; Figure~\ref{fig-selftest-time-series}).
Median relative errors for recent and native assessment summaries ranged
from -2.1\% to 3.5\%; across the selected fitted recruitment-scale,
growth and stock--recruitment parameters they ranged from -3.85\% to
2.00\% (Figure~\ref{fig-selftest-recovery}). Consistent with the purpose
of a self-test (Deroba et al., 2015; Kim et al., 2024), these results
show that the configured model can recover its principal derived
quantities from simulated observations without consistent large bias.
They do not test structural realism or form confidence intervals for the
assessment.
...
data sources. CPUE, and to a lesser extent length-frequency data,
favoured lower biomass, whereas tag data favoured higher biomass
(Figure~\ref{fig-diag-profile-data-sources}). CAAL data favoured higher
biomass than CPUE and length-frequency data but had a comparatively flat
profile across much of the range containing the overall minimum. The
component figures show the CPUE indices
(Figure~\ref{fig-diag-profile-data-cpue}), length-frequency
contributions by region (Figure~\ref{fig-diag-profile-data-lf}), CAAL
contributions by region (Figure~\ref{fig-diag-profile-data-caal}) and
tag programmes (Figure~\ref{fig-diag-profile-data-tag-prog}). This
conflict was represented in the uncertainty ensemble through alternative
tag-mixing periods, reporting-rate treatment during mixing and tag
overdispersion (\(\tau\)).
...
Among tagging programmes, the tropically focused PTTP made the strongest
contribution favouring higher biomass. RTTP was less influential but
also tended towards higher biomass, whereas JPTP in Region 1 favoured
lower biomass. The programmes are spatially separated and represent
...
The constant-recruitment age-structured production model (ASPM),
conditioned on observed catches and fitted only to the five CPUE
indices, provided a reduced comparison with the integrated diagnostic
model (Figure~\ref{fig-aspm-comparison}). In 2024 the ASPM estimated
...
integrated-model results used to construct the uncertainty ensemble.

\subsubsection{No-tagging and alternative-movement
comparisons}\label{sec-no-tag-movement-results}

Three additional comparisons removed tagging data and either fixed
...
A summary of diagnostic-model dynamic spawning depletion, spawning
potential, recruitment and fishing mortality relative to \(F_{MSY}\) is
provided in Figure~\ref{fig-diag-time-series}.
...
The diagnostic model fixed the Lorenzen natural-mortality scale at
\(M_{40}=0.07814\) per quarter. The longevity-based prior followed the
meta-analytical approach of Hamel (2015) and the updated longevity
formulation of Hamel \& Cope (2022); transforming its adult-mortality
median to the assessment-model \(M_0\) scale gave \(M_{40}=0.08082\).
Applying that value to the same 2026 growth schedule and Lorenzen
exponent produces a curve very close to the diagnostic one, whereas the
external tag estimate (\(M_{40}=0.0624\); 90\% CI 0.0500--0.0749)
produces lower mortality throughout the age range
(Figure~\ref{fig-natm-at-age}). All three curves decline steeply for
young fish and approach their respective fixed scales at age 40
quarters; the diagnostic choice is represented as an uncertainty axis
rather than estimated internally.

\subsubsection{Maturity}\label{sec-maturity-at-age}

Maturity at age was derived by applying the estimated 2026 growth curve
to the fixed maturity-at-length relationship used in the 2023 diagnostic
model. The resulting maturity-at-age curve differs slightly from that in
the 2023 diagnostic model because the growth curves differ
(Figure~\ref{fig-maturity-ogive}).

\subsubsection{Growth}\label{sec-growth-param}

The diagnostic-model growth curve is shown in
Figure~\ref{fig-growth-curve}. Estimated mean length was 25.68 cm at the
youngest age (SD 4.03 cm) and 156.43 cm at the oldest age (SD 14.32 cm);
...
Total annual recruitment fluctuated widely but was generally between
approximately 30 and 120 million individuals
(Figure~\ref{fig-recruitment-annual}). Estimated recruitment was lower
from the 1960s to the 1990s and subsequently fluctuated mostly between
40 and 120 million individuals per year. Regional patterns also varied
substantially through time. Region 1 declined sharply from the 1950s to
around 1980 and then stabilised, whereas Regions 2 and 3 generally
increased after 1980. Regions 1 and 3 contributed the largest shares of
recruitment; the seasonal regional allocation is shown in
Figure~\ref{fig-recruitment-regions}.

The fit to the stock recruitment relationship for the 2026 diagnostic
model is shown in Figure~\ref{fig-stock-rr}.

\subsubsection{Spawning potential and depletion: diagnostic
model}\label{sec-biomass-diag}

The 2026 diagnostic model estimated a steep initial decline in spawning
potential from the late 1950s to the mid-1980s, followed by a more
stable period to the late 1990s and a further decline to approximately
2010. Spawning potential was comparatively stable thereafter, with a
small decline in the terminal year (Figure~\ref{fig-diag-time-series}).
The long-term pattern is consistent with the CPUE indices
(Figure~\ref{fig-cpue-fit-residuals}) and the catch history, which
declined and then stabilised after approximately 2010
...
which it levels off just below 0.2 Figure~\ref{fig-diag-time-series}. As
...
The regional total-biomass comparison under the fitted fishing history
...
since 2010 Figure~\ref{fig-diag-time-series}.

Differences over time in the estimated adult and juvenile fishing
mortality are shown in Figure~\ref{fig-juv-adult-F}, with a gradual
...
also higher historic \(SB\) (Figure~\ref{fig-sensitivity-CAAL}).
...
The choice of the global-CPUE period used for the regional scaling
penalty affected \(SB\)/\(SB_{F=0}\), \(SB\), fishing mortality and
recruitment. Applying the full CPUE series rather than the shorter
period of maximum spatial coverage used in the diagnostic model
(1965--1969, as recommended by PAW) produced a higher population scale,
lower fishing mortality, higher recruitment and less depletion
(Figure~\ref{fig-sensitivity-regional-scaling}).
...
Activating or deactivating tag-reporting-rate adjustments during the
mixing period had little effect on \(SB\)/\(SB_{F=0}\), \(SB\),
recruitment or fishing mortality. This was unexpected and may reflect
the downweighting of tag data when \(\tau=2\); the stepwise analysis
showed a larger effect when \(\tau=1\)
(Figure~\ref{fig-sensitivity-premixing-tag-reporting-rate}).
...
mortality decreased (Figure~\ref{fig-sensitivity-tag-overdispersion}).
...
Of the 100 planned ensemble configurations, 80 met the
maximum-gradient-component (MGC) criterion \(\leq 1.0 \times 10^{-4}\),
of which 62 had positive-definite Hessians. The 80 models were retained
in the ensemble for management advice; ten did not meet the MGC
criterion and ten were incomplete. For the 62 positive-definite Hessian
fits, 100 correlated inverse-Hessian draws were propagated jointly by
the multivariate delta method; 18 Near-PDH fits contribute structural
...
in Figure~\ref{fig-ensemble-design-retention}. More models were retained
at the two higher \(\tau\) values, which assign less weight to tag data,
and when reporting-rate adjustments were excluded during the mixing
period. The balanced design contained 50 models per reporting-rate
treatment, but the retained set contained 34 inclusion and 46 exclusion
models. Under inclusion, MFCL reconstructs pre-mixing tag catches by
dividing by the estimated reporting rate; values approaching zero can
create very large intermediate values and numerical overflow (Davies et
al., 2026b). The imbalance therefore reflects differential numerical
stability, not greater scientific support for exclusion. Every retained
model received equal model weight, and the unequal retention is reported
explicitly rather than used as an additional weighting criterion.

The influence of each uncertainty component on the management quantities
across the ensemble of models is displayed in
Figure~\ref{fig-management-uncertainty-continuous-axes-steep-M},
Figure~\ref{fig-management-uncertainty-discrete-axes-tau-mix} and
...
The trajectories of \(SB\)/\(SB_{F=0}\), spawning potential, fishing
mortality and recruitment for each of the 80 models are displayed in
Figure~\ref{fig-ensemble-individual}. Fifteen central model estimates
had recent \(SB\)/\(SB_{F=0}<0.2\). Ribbon plots combining structural
uncertainty and the available estimation uncertainty are provided in
...
We do not present every ancillary result for every model. This section
relies mainly on the equal-model-weight ensemble summaries in
Table~\ref{tbl-manage-quant}, Table~\ref{tbl-cmm-depletion},
Table~\ref{tbl-stock-status-refs}, Table~\ref{tbl-ref-points} and
Table~\ref{tbl-est-uncertainty}, the time-series uncertainty in
Figure~\ref{fig-ensemble-est-uncert-ribbon}, and the dynamic Majuro and
Kobe plots in Figure~\ref{fig-time-dynamic-kobe-majuro}.

\subsubsection{Majuro and Kobe plots and comparisons with reference
points}\label{majuro-and-kobe-plots-and-comparisons-with-reference-points}

Most of the combined uncertainty distribution lies in the Kobe and
Majuro areas corresponding to a stock that is not overfished and not
undergoing overfishing, although the distribution extends into less
favourable categories (Figure~\ref{fig-combined-kobe-majuro-status}).
...
CMM 2025-02 contains an objective to maintain the spawning biomass
depletion ratio at or above the average \(SB/SB_{F=0}\) for 2012--2015.
Across the 80 ensemble models, excluding estimation uncertainty, this
average was 28.8\%. The corresponding average for 2021--2024 was 27.8\%,
giving a recent-to-2012--2015 spawning depletion ratio of 96.7\%; stock
status was therefore slightly below the objective
(Table~\ref{tbl-cmm-depletion}). This comparison is distinct from the
LRP and MSY-based quantities.

The dynamic Majuro and Kobe trajectories for both the diagnostic model
and the annual ensemble median show declining stock status and
increasing fishing mortality since the 1950s, with fishing mortality
increasing more strongly from the late 1960s
(Figure~\ref{fig-time-dynamic-kobe-majuro}). The ensemble median remains
...
Dynamic MSY declined from relatively high values early in the assessment
...
decades (Figure~\ref{fig-dynamic-msy}). Recent catches were generally
...
Estimated fishery impact increased in all regions, with the greatest
terminal impact in Region 2 and substantial impacts in Regions 3 and 4
(Figure~\ref{fig-impact-region}). At the stock-wide scale, associated
...
The diagnostic-model equilibrium yield curve was relatively flat near
its maximum (Figure~\ref{fig-yield}). The fishing-mortality multiplier
...
\subsubsection{Catch-based projections}\label{sec-projection-results}

Under fishery- and quarter-specific catches fixed at their 2022--2024
means, the ensemble projected recovery in both the LRP depletion metric
and spawning potential after 2024
(Figure~\ref{fig-projection-stock-trajectories}). Across the 800
model--recruitment realisations, the median LRP metric increased to
0.355 in 2030, 0.517 in 2040 and 0.545 in 2054; 5.2\% of realisations
were below the 0.20 LRP in 2030 and none were below it in 2040 or 2054
(Table~\ref{tbl-stock-projections}). Median spawning potential increased
from 523.1 thousand t in 2030 to 720.6 thousand t in 2054. These are
conditional projections under the specified recent-catch scenario, not
forecasts of future catches or management action, and their intervals
include recruitment variability but not Hessian parameter uncertainty.
...
The 2026 assessment is a substantial revision of the WCPO bigeye tuna
...
\begin{itemize}
\item
  \textbf{Optimisation and practical estimability.} This was the first
...
\item
  \textbf{Regional recruitment information.} Regional recruitment
...
\item
  \textbf{Length-composition weighting.} The selected DM setting
...
\item
  \textbf{Self-test.} The first formal self-test in this assessment
...
\item
  \textbf{Tagging and population scale.} Population scaling remained the
...
\item
  \textbf{Alternative movement and no-tagging models.} Removing tagging
...
The ASPM comparison produced a larger and less depleted terminal
...
The ensemble median \(SB_{recent}/SB_{F=0}\) was 0.277, with a central
80\% interval of 0.172--0.395, and the probability of this LRP quantity
being below 0.20 was 18.6\% (Table~\ref{tbl-manage-quant} and
Table~\ref{tbl-stock-status-refs}). Median \(F_{recent}/F_{MSY}\) was
0.838, although 24.1\% of the uncertainty distribution exceeded one. For
the separate CMM 2025-02 objective, mean spawning depletion was 27.8\%
in 2021--2024 and 28.8\% in 2012--2015 across the 80 central models,
excluding estimation uncertainty, giving a recent-to-2012--2015 spawning
depletion ratio of 96.7\% (Table~\ref{tbl-cmm-depletion}). Stock status
was therefore slightly below this objective. The central assessment
outcome is neither below the adopted LRP nor above \(F_{MSY}\), while
the uncertainty around those benchmarks remains important for management
interpretation.

The constant-recent-catch projections provide a conditional view of
future dynamics rather than a forecast. Under catches fixed at their
2022--2024 fishery- and quarter-specific means, the median LRP depletion
metric increased to 0.355 in 2030 and 0.517 in 2040
(Table~\ref{tbl-stock-projections}). This result depends on the assumed
catches, model ensemble and resampled recruitment process; it does not
account for implementation uncertainty, future changes in fishing
patterns or Hessian parameter draws.
...
\begin{itemize}
\item
  Develop and simulation-test a tag-recapture likelihood that treats
...
\item
  Investigate the lower-objective jitter solutions and weakly determined
...
\item
  Test the sensitivity of regional recruitment to the amount and size
...
\item
  Re-evaluate the eight DM groups and \(N_{\max}\) with
...
\item
  Continue evaluation of the regional CPUE-scaling penalty and its
...
\item
  Extend self-tests to structural cross-tests and the tag-likelihood
...
\item
  Establish a separate MFCL continuous-integration project that
  exercises assessment-critical flags and their interactions on small
  verified models, including \(\tau\), pre-mixing reporting-rate
  treatment and mixing-period controls. Automated checks of expected
  likelihood components, fitted outputs and numerical failures would
  reduce the risk that complex or poorly documented settings affect an
  assessment unnoticed.
\end{itemize}
...
\item
  Median recent depletion (2021--2024; \(SB_{recent}\)/\(SB_{F=0}\)) is
  0.277 (central 80\% interval including structural and available
  estimation uncertainty: 0.172--0.395; central 95\% interval:
  0.136--0.442).
\item
  Fifteen of the 80 central model estimates had
  \(SB_{recent}\)/\(SB_{F=0}<0.2\); after including available estimation
  uncertainty, the probability below the LRP is 18.6\%.
\item
  CMM 2025-02 contains an objective to maintain spawning depletion at or
  above its 2012--2015 average. Across the 80 central models, excluding
  estimation uncertainty, mean depletion was 27.8\% in 2021--2024 and
  28.8\% in 2012--2015, giving a recent-to-2012--2015 spawning depletion
  ratio of 96.7\%; stock status was therefore slightly below the
  objective (Table~\ref{tbl-cmm-depletion}).
\item
  Median recent fishing mortality (2020--2023),
  \(F_{recent}\)/\(F_{MSY}\), is 0.838 (central 80\% interval:
  0.592--1.206; central 95\% interval: 0.460--1.351).
\item
  Although the stock is assessed as more depleted and subject to higher
  fishing mortality than in 2023, the ensemble indicates that it is not
  overfished under the adopted LRP criterion and is not undergoing
  overfishing based on median \(F_{recent}/F_{MSY}<1\).
\item
  Overall, the 2026 assessment presents a more pessimistic stock status
  than the 2023 assessment.
\end{itemize}

\section{Acknowledgements}\label{acknowledgements}

We thank WCPFC members, participating territories and other providers of
fisheries data, and the observers, port samplers, tagging teams and
fishers who collected the biological and tagging observations. We also
acknowledge the 2026 Pre-assessment Workshop participants; the Pacific
Community data, assessment and computing-support teams; and the SPC
sclerochronology laboratory team, which aged many additional otoliths
for this assessment. We thank the Japan Fisheries Research and Education
Agency (FRA) for providing additional age--length data from the North
Pacific. This assessment was supported through the WCPFC regular
Scientific Services and Additional Resourcing to SPC budget lines,
including funding for the additional stock-assessment scientist who led
this assessment. CPUE data preparation was also supported by the
European Union's Pacific--European Union Marine Partnership Programme
Phase 2. K. Kim especially thanks his family for their understanding of
the long hours required to complete the assessment.

\section{Statement on the use of generative
AI}\label{statement-on-the-use-of-generative-ai}

GitHub Copilot, a generative artificial-intelligence assistant, was used
to support code development and checking and the preparation of selected
...
\caption{\label{tbl-open-science-resources}Public assessment outputs, source repositories and pinned build resource.}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.62\linewidth}>{\raggedleft\arraybackslash}p{0.20\linewidth}@{}}

\caption{\label{tbl-cmm-depletion}Recent spawning depletion relative to the 2012--2015 average across the 80 equal-weight central models, excluding estimation uncertainty. For each year, depletion is spawning biomass divided by mean unfished spawning biomass over the preceding ten years. The period values are arithmetic means across models; the final row is their ratio.}
...
\bottomrule
\endlastfoot
$D_{\mathrm{recent}}$ (2021--2024) & 0.278 \\
$D_{2012\text{--}2015}$ & 0.288 \\
$D_{\mathrm{recent}}/D_{2012\text{--}2015}$ & 0.967 \\
...
\caption{\label{tbl-stock-status-refs}Stock status probabilities in relation to the adopted limit and MSY-based reference points.}
...
\begin{longtable}{@{}rrrrr>{\raggedleft\arraybackslash}p{0.22\linewidth}rrr@{}}

\caption{\label{tbl-stock-projections}Projected management quantities at selected years across 80 models and ten recruitment sequences per model. The LRP metric is the four-year mean spawning biomass divided by the preceding ten-year mean unfished spawning biomass; the LRP is 0.20. Catches are fixed by fishery to their 2022--2024 means, and intervals exclude Hessian parameter uncertainty.}

\tabularnewline

\\
\toprule
Year & $D$ 10\% & $D$ median & $D$ 90\% & Below LRP & Spawning median ($10^3$ MT) & $C/\mathrm{MSY}$ 10\% & $C/\mathrm{MSY}$ median & $C/\mathrm{MSY}$ 90\% \\
\midrule
\endfirsthead
\toprule
Year & $D$ 10\% & $D$ median & $D$ 90\% & Below LRP & Spawning median ($10^3$ MT) & $C/\mathrm{MSY}$ 10\% & $C/\mathrm{MSY}$ median & $C/\mathrm{MSY}$ 90\% \\
...
\bottomrule
\endlastfoot
2030 & 0.225 & 0.355 & 0.514 & 5.2\% & 523.1 & 0.638 & 0.811 & 1.003 \\
2040 & 0.409 & 0.517 & 0.628 & 0.0\% & 690.5 & 0.642 & 0.842 & 1.052 \\
2054 & 0.455 & 0.545 & 0.641 & 0.0\% & 720.6 & 0.648 & 0.847 & 1.053 \\
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/spatial-structure-comparison.png}

}

\caption{\label{fig-map-spatial-comparison}Comparison of the spatial
structures used in the 2023 and 2026 diagnostic models. The 2026
structure consolidates the previous nine regions into five while
retaining an eastern equatorial region for the principal tag-recapture
information.}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/age-data-growth-by-region.png}

}

\caption{\label{fig-growth-curve}Observed age--length data and fitted
mean length at age for the five model regions and all regions combined.
Circle area is proportional to the number of aged fish. Shading is the
fitted mean plus or minus 1.96 length-at-age standard deviations and
represents fish-level length variability, not parameter-estimation
uncertainty.}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/maturity-comparison.png}

}

\caption{\label{fig-maturity-ogive}Maturity-at-age schedules in the 2023
and 2026 diagnostic models. The maturity-at-length relationship was
unchanged; the small difference results from applying it to the updated
2026 growth curve.}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/natural-mortality-evidence.png}

}

\caption{\label{fig-natm-at-age}Age-specific quarterly natural mortality
obtained by applying three assessment-relevant \(M_0\) values to the
same 2026 diagnostic growth schedule and fixed Lorenzen exponent (-1).
Curves show the diagnostic value (0.07814), the model-aligned
longevity-prior median (0.08082) derived following Hamel (2015) and
Hamel \& Cope (2022), and the tag estimate from Ducharme-Barth et al.
(2026) (0.0624). Shading is the tag estimate's 90\% confidence interval
(0.0500--0.0749) propagated over age. Open the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-ensemble/bet-2026-ensemble-report.html\#natural-mortality-evidence}{ensemble
report}.}
...
\caption{\label{fig-cpue-fit-residuals}Observed and fitted relative CPUE
...
\includegraphics[width=1\linewidth,height=0.72\textheight]{figures_new/length-frequency-fit-by-fishery.png}

}

\caption{\label{fig-length-fit-by-fishery}Observed (bars) and fitted
...
\caption{\label{fig-tag-attrition-programmes}Observed (black points) and
...
\caption{\label{fig-tag-reporting-rates-active}Fitted tag-reporting
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/jitter-diagnostics-diagnostic-model.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/jitter-derived-diagnostic-model.png}
...
the dark line is the jitter median with its terminal value marked by a
circle, and the red line is the diagnostic model without jitter.
Recruitment is in millions of fish and spawning potential is in thousand
metric tonnes. Open the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-jitter/jitter-report.html}{interactive
jitter report}.}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/jitter-regional-depletion-diagnostic-model.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/selftest-key-recovery-diagnostic.png}

}

\caption{\label{fig-selftest-recovery}Relative recovery error for recent
management and assessment quantities and selected estimated parameters
in the diagnostic model. Points are the 50 pseudo-data refits, boxes
show their empirical distributions, the red line marks exact recovery
and grey dashed lines mark errors of -5\% and +5\%. Open the
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/selftest-recovery-diagnostic.png}

}

\caption{\label{fig-selftest-time-series}Self-test recovery of annual
assessment quantities. The red line is the generating value, the
dark-blue line is the median across 50 replicate refits and the nested
blue bands are the central 50\%, 80\% and 95\% empirical ranges. Open
the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-selftest/selftest-report.html}{interactive
self-test report}.}
...
black line is the total CPUE contribution; the coloured lines and
matching heatmap rows show the five regional indices. Larger
\(\Delta\)NLL indicates a poorer fit at that biomass, and the dashed
line marks \(\Delta\)NLL = 1.92. Select individual indices in the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-diagnostic/bet-2026-likelihood-profile-viewer.html}{interactive
profile viewer}.}
...
profile by model region. The black line is the total length-frequency
contribution; the coloured lines and matching heatmap rows show the five
regions. Larger \(\Delta\)NLL indicates a poorer fit at that biomass,
and the dashed line marks \(\Delta\)NLL = 1.92. Open a region in the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-diagnostic/bet-2026-likelihood-profile-viewer.html}{interactive
profile viewer} for fishery-level detail.}
...
likelihood profile by model region. The black line is the total CAAL
contribution; the coloured lines and matching heatmap rows show the five
regions. Larger \(\Delta\)NLL indicates a poorer fit at that biomass,
and the dashed line marks \(\Delta\)NLL = 1.92. Open a region in the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-diagnostic/bet-2026-likelihood-profile-viewer.html}{interactive
...
\caption{\label{fig-diag-profile-data-tag-prog}Tag likelihood profile by
tagging programme. The black line is the total tag contribution; the
coloured lines and matching heatmap rows show RTTP, PTTP and JPTP.
Larger \(\Delta\)NLL indicates a poorer fit at that biomass, and the
dashed line marks \(\Delta\)NLL = 1.92. Open a programme in the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-diagnostic/bet-2026-likelihood-profile-viewer.html}{interactive
profile viewer} for release-group detail.}
...
all 33 fisheries. Each panel shows one fitted fishery-specific
cubic-spline curve; panel labels give the fishery ID and definition.}
...
length for all 33 fisheries. Each panel shows one fishery, with
age-specific selectivity mapped to the fitted mean length at age.}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/stock-recruitment.png}

}

\caption{\label{fig-stock-rr}Fitted Beverton--Holt stock--recruitment
relationship for the 2026 diagnostic model. Points show annual
recruitment and adult-biomass estimates and are coloured by year. Open
the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-diagnostic/bet-2026-diagnostic-report.html\#fig-stock-recruitment}{diagnostic
report}.}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-steepness.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-natural-mortality.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-conditional-age-at-length.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-effort-creep.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-regional-scaling.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-tag-mixing-periods-ks-d-statistic-cutoff.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-pre-mixing-tag-reporting.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures_new/sensitivity-tag-overdispersion.png}
...
\caption{\label{fig-projection-stock-trajectories}All-region LRP
depletion metric (a) and spawning potential in thousand metric tonnes
(b), joining estimates through 2024 to projections for 2025--2054. The
LRP metric is the four-year mean spawning biomass divided by the
preceding ten-year mean no-fishing biomass. Medians and central 50\%,
80\% and 95\% intervals use all 800 model--recruitment realisations; ten
thin lines show individual projection realisations. Historical intervals
show structural uncertainty; projections add recruitment variability
without Hessian parameter draws. The 0.20 line in panel a is the
stock-wide LRP. Open the
\href{https://pacificcommunity.github.io/ofp-sam-bet-2026-ensemble/bet-2026-ensemble-interactive-viewer.html}{interactive
ensemble viewer}.}
...
\subsection{No-tagging and alternative-movement model
comparisons}\label{sec-appendix-no-tag-movement}

The current bigeye tuna diagnostic model includes tagging data
...
To determine the impact of tagging data on the assessment results, three
alternative models were developed and compared with the diagnostic
model:

\begin{itemize}
\item
  \textbf{5R-MFCL:} the diagnostic model with tagging data removed and
...
\item
  \textbf{5R-SEAP:} a model similar to 5R-MFCL, but with movement
  parameters supplied by the current reference version of the bigeye
  tuna SEAPODYM model (Senina et al., 2026).
\item
  \textbf{1R:} a single-region ``fleets-as-areas'' bigeye tuna model
  without tagging data (Hampton et al., 2026b).
\end{itemize}

The fixed movement estimates used by 5R-MFCL and 5R-SEAP are shown in
...
\includegraphics[width=0.92\linewidth,height=0.78\textheight]{figures_new/appendix-mfcl-movement.png}
...
\includegraphics[width=0.92\linewidth,height=0.78\textheight]{figures_new/appendix-seapodym-movement.png}
...
Estimates of recruitment, spawning biomass, spawning-biomass depletion
...
Overall, the headline estimates were reasonably consistent across
models:

\begin{itemize}
\item
  \textbf{Recruitment (1952--2024):} the 5R-Diag average was 17.3
  million fish per quarter. The 5R-MFCL average was 14\% higher, 5R-SEAP
  was almost identical and 1R was 8\% lower.
\item
  \textbf{Spawning biomass (1952--2024):} the 5R-Diag average was 597
  thousand t. The 5R-MFCL average was 9\% higher, 5R-SEAP was 22\% lower
  and 1R was 24\% higher.
\item
  \textbf{Spawning-biomass depletion (2020--2024):} the 5R-Diag average
  was 0.18. The 5R-MFCL average was 9\% higher, 5R-SEAP was 15\% lower
  and 1R was 5\% higher.
\item
  \textbf{Regional spawning-biomass distribution (1952--2024):} 5R-MFCL
...
\includegraphics[width=0.92\linewidth,height=0.78\textheight]{figures_new/appendix-no-tag-model-comparison.png}
...
\subsubsection{Conclusions}\label{sec-appendix-no-tag-conclusions}

The key estimates from 5R-Diag and the no-tagging model with the same
...
SEAPODYM estimates somewhat greater movement and mixing than the
...
Despite the different bases for estimating movement in MFCL and
...
\bibitem[\citeproctext]{ref-carvalho_diagnostic_2017}
Carvalho, F., Punt, A. E., Chang, Y.-J., Maunder, M. N., \& Piner, K. R.
(2017). Can diagnostic tests help identify model misspecification in
integrated stock assessments? \emph{Fisheries Research}, \emph{192},
28--40. \url{https://doi.org/10.1016/j.fishres.2016.09.018}

\bibitem[\citeproctext]{ref-carvalho_cookbook_2021}
...
assessments. \emph{Fisheries Research}, \emph{240}, 105959.
...
\bibitem[\citeproctext]{ref-deroba_simulation_2015}
...
the {ICES} strategic initiative on stock assessment methods. \emph{ICES
Journal of Marine Science}, \emph{72}(1), 19--30.
...
\bibitem[\citeproctext]{ref-kim_quantitative_2025}
Kim, K., \& Neubauer, P. (2025). A quantitative evaluation of the
pollock {Gadus chalcogrammus} restocking programme in south korea.
\emph{Canadian Journal of Fisheries and Aquatic Sciences}, \emph{82},
1--20. \url{https://doi.org/10.1139/cjfas-2024-0375}

\bibitem[\citeproctext]{ref-kim_enhancing_2024}
Kim, K., Sibanda, N., Arnold, R., \& A'mar, T. (2024). Enhancing
data-limited assessments with random effects: A case study on korea chub
mackerel ({Scomber japonicus}). \emph{Canadian Journal of Fisheries and
Aquatic Sciences}, \emph{81}(10), 1433--1455.
\url{https://doi.org/10.1139/cjfas-2023-0358}

\bibitem[\citeproctext]{ref-kolody_evaluation_2014}
Kolody, D., \& Hoyle, S. (2015). Evaluation of tag mixing assumptions in
western pacific ocean skipjack tuna stock assessment models.
\emph{Fisheries Research}, \emph{163}, 127--140.
\url{https://doi.org/10.1016/j.fishres.2014.05.008}
...
\bibitem[\citeproctext]{ref-maunder_contemporary_2015}
Maunder, M. N., \& Piner, K. R. (2015). Contemporary fisheries stock
assessment: Many issues still remain. \emph{ICES Journal of Marine
Science}, \emph{72}(1), 7--18.
...
\bibitem[\citeproctext]{ref-minte-vera_auxiliary_2021}
Minte-Vera, C. V., Maunder, M. N., \& Aires-da-Silva, A. (2021).
Auxiliary diagnostic analyses used to detect model misspecification and
highlight potential solutions in stock assessments: Application to
yellowfin tuna in the eastern {Pacific} {Ocean}. \emph{ICES Journal of
Marine Science}, \emph{78}(10), 3521--3537.
\url{https://doi.org/10.1093/icesjms/fsab213}
...
\bibitem[\citeproctext]{ref-senina_updated_2026}
Senina, I., Bonnin, L., Forestier, R., Lengaigne, M., Guihou, K.,
Temple-Boyer, J., \& Nicol, S. (2026). \emph{Updated {SEAPODYM}
reference models for four pacific tuna species} (No.
{WCPFC}-{SC}22-2026/{EB}-{IP}-04). WCPFC.
...
Subbey, S. (2018). Parameter estimation in stock assessment modelling:
Caveats with gradient-based algorithms. \emph{ICES Journal of Marine
Science}, \emph{75}(5), 1553--1559.
