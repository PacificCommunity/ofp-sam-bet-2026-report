...
Penalties favoured lower biomass. The component figures show the CPUE
...
components (Figure~\ref{fig-diag-profile-data-penalty}). This conflict
was represented in the uncertainty ensemble through alternative
...
\(M_{40}=0.07814\) per quarter. Applying the same 2026 growth schedule
and Lorenzen exponent to the model-aligned Hamel--Cope median
(\(M_{40}=0.08082\)) produces a very similar age-specific curve, whereas
the external tag estimate (\(M_{40}=0.0624\); 90\% CI 0.0500--0.0749)
...
We thank WCPFC members, cooperating non-members and participating
...
\caption{\label{tbl-open-science-resources}Public assessment outputs,
source repositories and pinned build resource.}
...
Pinned build environment & \inlinecode{tuna-flow-private:v2.7} &
\href{https://github.com/PacificCommunity/ofp-sam-bet-2026-report/blob/rev1/.github/workflows/render-quarto.yml}{Report
workflow and full digest} (\inlinecode{sha256:4fee4c40...9c8852}) \\
...
Hamel--Cope median (0.08082), and the tag estimate from Ducharme-Barth
et al. (2026) (0.0624). Shading is the tag estimate's 90\% confidence
interval (0.0500--0.0749) propagated over age. Open the
...
\caption{\label{fig-diag-profile-data-tag-prog}Tag likelihood profile.
...
relationship for the 2026 diagnostic model. Filled circles are annual
recruitment and adult-biomass estimates used in the fit and are coloured
by year; open circles denote years before 1968, which were not used to
fit the relationship. Open the
...
reference models for four pacific tuna species}
