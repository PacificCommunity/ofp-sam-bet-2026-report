...
Pre-Assessment Workshop (PAW) (P. Hamer 2026). Readers are also referred
to SC22-SA-IP09 (J. Hampton et al. 2026) which includes exploration of a
...
\item
  ...
  addition of observed length data for longline fisheries (T. Peatman,
  Kim, et al. 2026)
\item
  Stricter size composition data filtering and enhanced reweighting
  approach (T. Peatman, Kim, et al. 2026)
\item
  Global sdmTMB CPUE analysis used for regional scaling, and new MFCL
  regional scaling penalty (T. Teears 2026; N. Davies et al. 2026a)
\item
  ...
  (PAW and yellowfin tuna peer review recommendation) (P. Hamer 2026; T.
  Teears 2026; A. E. Punt, Maunder, and Ianelli 2023)
\item
...
\item
  ...
  model (yellowfin tuna peer review A. E. Punt, Maunder, and Ianelli
  (2023)), and removal of Coral Sea Tagging data (supported by PAW)
\item
  Tag reporting rate penalties informed by increased tag seeding (T.
  Peatman et al. 2026)
\item
  Substantial increase in age at length data, including north Pacific
  samples (Wakefield et al. 2026; Hasegawa et al. 2026)
\item
...
for longline fisheries to provide regional abundance indices (T. Teears
2026), revised analysis of tagger effects and tag reporting rates (T.
Peatman, Scutt Phillips, et al. 2026; T. Peatman et al. 2026), enhanced
and improved size composition data analyses and preparation, include the
weight data to length conversion work (T. Peatman, Kim, et al. 2026),
tag mixing simulation modeling (J. Scutt Phillips, Senina, and Bonin
2026) and developments to MFCL software (N. Davies et al. 2026b). An
extensive process of stepwise model development was conducted to
transition from the 2023 to the 2026 assessment model and new data.
...
been conducted regularly since the late 1990s (Langley et al. 2008; S.
Harley, Hoyle, and Langley 2009; S. J. Harley et al. 2010; N. Davies et
al. 2011; Shelton J. Harley et al. 2014-08; S. McKechnie, Pilling, and
Hampton 2017; M. Vincent, Pilling, and Hampton 2018; Nicholas
Ducharme-Barth et al. 2020; Day et al. 2023). As in previous
...
the general approach of integrated modeling (D. Fournier and Archibald
...
D. A. Fournier, Hampton, and Sibert (1998); J. Hampton and Fournier
(2001); N. Davies et al. (2026c)). MFCL implements a size-based, age-
and spatially-structured population model. Model parameters are
estimated by maximizing an objective function, consisting of both
likelihood (data) and ``prior''\footnote{Note that unless otherwise
  stated any mention of a ``prior'' in this report does not refer to a
  prior in the Bayesian sense, though the effect on the parameter
  estimate is similar, but rather a penalty placed on the likelihood
  such that the estimated parameter does not deviate too much from the
  specified ``prior'' value. The magnitude of the deviation from the
  ``prior'' is dependent on the information content of the data and the
  strength of the likelihood penalty applied.} information components
(penalties).
...
(P. Hamer 2026) guide this ongoing process. Changes to aspects of an
...
data inputs (T. Teears 2026), the paper on size composition data
preparations and weighting (T. Peatman, Kim, et al. 2026), the papers on
tag reporting rates and tagger effects estimations (T. Peatman, Scutt
Phillips, et al. 2026; T. Peatman et al. 2026), the paper on tag mixing
estimation (J. Scutt Phillips, Senina, and Bonin 2026), the paper on
MFCL developments (N. Davies et al. 2026b) and the paper on conceptual
models of bigeye and yellowfin population structure (P. Hamer et al.
2023). Planning and approaches for this assessment were informed by the
discussion at the 2026 SPC PAW (P. Hamer 2026). Several other papers are
also relevant to this assessment including the single region, no tag
data, fleets-as-areas model experiments for yellowfin and bigeye tuna
(J. Hampton et al. 2026), and an experiment exploring implications of
growth variation on the 2023 bigeye tuna assessment (J. Hampton, Davies,
and Hamer 2026).
...
\item
  ...
  length data for longline fisheries (T. Peatman, Kim, et al. 2026)
\item
  Stricter size composition data filtering and enhanced reweighting
  approach (T. Peatman, Kim, et al. 2026)
\item
...
\item
  ...
  yellowfin tuna peer review recommendation) (P. Hamer 2026; T. Teears
  2026; A. E. Punt, Maunder, and Ianelli 2023)
\item
  External M-scale assumption rather than internal estimation (PAW
  recommendation to improve model stability)
\item
  Release group specific tag maxing based on individual based tag mixing
  model (yellowfin tuna peer review A. E. Punt, Maunder, and Ianelli
  (2023)), removal of Coral Sea Tagging data
\item
  Tag reporting rate penalties informed by increased tag seeding (T.
  Peatman et al. 2026)
\item
...
Indo-Pacific regions (Chow et al. 2000; Gonzalez, Beerli, and Zardoya
2008; Grewe et al. 2019). There is no strong evidence for genetic
population structure in the Pacific Ocean (Grewe and Hampton 1998;
Bradley R. Moore et al. 2020-10; Natasha et al. 2022-04). While genetic
studies are largely uninformative on the rates of mixing of bigeye tuna
throughout the Pacific, they are broadly consistent with the results of
tagging experiments conducted by SPC and the Inter-American Tropical
Tuna Commission (IATTC) (K. Schaefer et al. 2015; Bradley R. Moore et
al. 2020-10). These studies show that while the majority of tagged
bigeye have been recaptured in the general regions of their release
(i.e.~within 1,500 nautical miles) some tagged bigeye move large
distances across the Pacific Ocean (i.e.~\textgreater4,000 nautical
miles) Figure Figure~\ref{fig-tag-displacement-map}. These occasional
...
tropical tag release sites and temperate zones (K. Schaefer et al. 2015;
Bradley R. Moore et al. 2020-10; S. McKechnie, Pilling, and Hampton
2017). While there is apparent high movement throughout the equatorial
Pacific and lack of genetic structure, this does not preclude spatial
structure in population processes that are important in stock assessment
(i.e., growth, mortality, recruitment) and regional fishing
impacts/local depletion. Growth rates have been shown to differ between
bigeye tuna in the WCPO and EPO, with smaller lengths-at-age observed
for bigeye in the WCPO, and suggestion of a clinal increase in growth
rates from the west to east Pacific (S. McKechnie et al. 2015;
Aires-da-Silva et al. 2015; J. Farley et al. 2017). This is also
reflected in observations of increased size at maturity for bigeye in
the EPO compared to the WCPO (J. Farley et al. 2017). Further, there is
some evidence from otolith chemistry studies for structuring of
recruitment sources. For example, Rooker et al. (2016-05) indicated that
1--2 year old fish sampled from the Marshall Islands were derived from
local sources, whereas fish caught off Hawai'i were suggested to have
originated more broadly from the central equatorial region. In turn,
otolith chemistry and genetic studies of juvenile bigeye in the
Indonesian archipelagic waters suggested juvenile residency and low
mixing with the broader western Pacific for this life stage (Bradley R.
Moore et al. 2019-12; Proctor et al. 2019).

The paper by P. Hamer et al. (2023) considers published information on
...
sensitivity analysis by S. McKechnie et al. (2015) in 2015 showed that
...
(Langley et al. 2008; S. Harley, Hoyle, and Langley 2009; S. J. Harley
et al. 2010; N. Davies et al. 2011; Shelton J. Harley et al. 2014-08; S.
McKechnie, Pilling, and Hampton 2017; M. Vincent, Pilling, and Hampton
2018; Nicholas Ducharme-Barth et al. 2020; Day et al. 2023).
...
(FL) of about 200 cm (Aires-da-Silva et al. 2015; J. Farley et al.
2017), with an estimated average maximum length of 157 cm in the WCPO
(Eveson et al. 2020; J. Farley et al. 2020). The growth parameters for
bigeye tuna are influential biological parameters in the stock
assessment (S. McKechnie, Pilling, and Hampton 2017; M. Vincent,
Pilling, and Hampton 2018). Studies to improve growth information for
bigeye have occurred in recent years, notably by Eveson et al. (2020);
J. Farley et al. (2020); Wakefield et al. (2026); Hasegawa et al.
(2026). Recent bomb radiocarbon age work has validated the
interpretation of growth bands in bigeye otoliths as annual and
...
WCPO is approximately 15 years (J. Farley et al. 2020). The 2026
...
the previous assessments (Wakefield et al. 2026; Hasegawa et al. 2026).

Bigeye tuna in the WCPO become reproductively active from about 80 cm
FL, and nearly all individuals \textgreater120 cm FL are mature (J.
Farley et al. 2017). There is some evidence for regional variation in
maturity-at-length in the WCPO and EPO (Nicol et al. 2011; J. Farley et
al. 2017; K. M. Schaefer, Fuller, and Miyabe 2005). As with other tunas,
the population sex ratio of bigeye tuna changes at around the age/size
of reproductive maturity to increasingly favor males at larger sizes,
thought to relate to higher mortality of females due to the energetic
stress of regular spawning Sun, Chu, and Yeh (2006); Sam McKechnie,
Tremblay-Boyer, and Pilling (2017)). Sex ratio information is used to
define spawning potential (rather than spawning biomass) as a product of
maturity status, female sex-ratio in the population, and fecundity.
Spawning typical occurs in water temperatures above 24\(^\circ\) C, with
an approximate daily cycle, and occurs all year in the tropical Pacific,
but more seasonally in the warmer months in sub-tropical regions (Sun,
Chu, and Yeh 2006; J. Farley et al. 2017). Areas of higher larval
densities have been found in the western Pacific from Papua New Guinea
to Philippines, Chinese Taipei to the Okinawa Islands, also several
areas in the central Pacific, and in the eastern Pacific off Panama and
Colombia (Nishikawa et al. 1985; Ijima and Jusup 2023-04).
...
(\textless2 years) (J. Hampton 2000). The 2020 bigeye assessment
conducted a life-history based meta-analysis (Piner and Lee 2011) of
natural mortality for bigeye and indicated an envelope of potential
average quarterly M rates of lower 95\% confidence interval (0.109),
mean (0.127) and upper 95\% confidence interval (0.146) (Nicholas
Ducharme-Barth et al. 2020). The 2023 assessment estimated M internally
to the MFCL model using a Lorenzen function, adult (age 40 quarters) M
was estimated at 0.078 per quarter (Day et al. 2023).
...
research over recent years (T. Peatman et al. 2019). This purse seine
...
assessment which excludes the overlap area with IATTC) (S. McKechnie,
Pilling, and Vidal 2026). This compares to the WCPO longline catch of
between approximately 32,000-59,000 mt over the same period (Hare et al.
2025; S. McKechnie, Pilling, and Vidal 2026).
...
Williams and Ruaia 2023; S. McKechnie, Pilling, and Vidal 2026). Over
the recent period (2015-2024), approximately 50\% of the catch by
weight, but 10\% by numbers was taken by longline, reflecting the
selection of longline gear for larger fish. In contrast, 50\% of the
catch by numbers, but only 35\% by weight was taken by purse seine sets
associated with FADs. A small purse seine fishery also operates in the
coastal waters off Japan with an annual bigeye catch of less than 500 mt
in recent years. A slightly higher level of bigeye catch is taken by the
coastal Japanese pole-and-line fishery relative to the coastal Japanese
purse seine, however, overall these catches are negligible.
...
recommended by the peer review of the 2020 yellowfin tuna assessment (A.
E. Punt, Maunder, and Ianelli 2023). Improvements in these data inputs
are ongoing and readers should refer to the companion papers highlighted
at the end of Section Section~\ref{sec-introduction} for detailed
descriptions of how the data and biological inputs were formulated as
only brief overviews are provided below. A summary of the data available
for the assessment is provided in (T. Teears 2026), and in Figure
...
2014, after the bigeye assessment peer review (Ianelli, Maunder, and
Punt 2012). This small region was included to better model the tagging
data from the historical Coral Sea tagging cruises (Shelton J. Harley et
al. 2014-08). Region 9 was maintained in the previous assessments,
partly because of tag-recaptures with long times at liberty that were
thought to provide information on natural mortality (M) which was
estimated internal to the model in the 2023 assessment. In the 2026
assessment, in an attempt to improve model stability, after discussion
at the PAW it was decided to return to the approach of earlier
assessments and apply M as an external parameter (P. Hamer 2026).
Related to this change the archival tag-recapture data from the Coral
Sea Tagging program combined with conventional tags from the Regional
Tuna Tagging Programme (RTTP) and the Pacific Tuna Tagging Programme
(PTTP) was used in an external analysis to estimate an adult M and
M-at-age (using previous growth curves) Lorenzen curve (N.
...
spatial complexity(P. Hamer 2026; P. Hamer et al. 2023). An alternative
5 region structure was also proposed at PAW with a separate central-east
...
region tag mixing. Ultimately based on the tag mixing modeling (J. Scutt
Phillips, Senina, and Bonin 2026), the 5 region structure was chosen for
the 2026 assessment Figure Figure~\ref{fig-map-5regions}. The PAW also
recommended to explore a single region fleets-as-area model, excluding
all tag data (P. Hamer 2026). This exploration is provided in a
companion paper to SC22 (J. Hampton et al. 2026).
...
assessment region (T. Teears 2026). In the 2023 assessment a single
sdmTMB CPUE standardisation model was conducted across the entire
assessment region (i.e.~global model), the quarterly predictions were
then aggregated and summed by model regions to generate the region
specific abundance indices (T. Teears et al. 2023), that were assumed to
have constant catchability and selectivity. This allowed MFCL to use the
...
indices (A. E. Punt, Maunder, and Ianelli 2023). This was discussed in
depth at the PAW, where it was recommended to run a global sdmTMB model
using the long-term Japanese operational data, use this global model for
...
model regions (P. Hamer 2026). This approached was adopted and required
considerable additional work in the CPUE analyses (T. Teears 2026), and
...
the time series abundance indices for the model to fit to (N. Davies et
al. 2026b). Furthermore, specification of the time period from the
...
(T. Teears 2026). The MFCL code was also modified to input global CPUE
...
temporal (quarter) resolution of the model (T. Peatman, Kim, et al.
2026). However, for the index fisheries, while the same aggregation
process is conducted, the size data are weighted by standardised CPUE
(rather than by catch) so that the size data are more representative of
the abundance of the underlying population in each region and time
period. Further, because the size data for the index and extraction
fisheries are effectively being used twice (but weighted differently),
the likelihood weighting for the size composition is adjusted such that
the original intended weight (effective sample size) in the likelihood
is preserved.
...
estimated from raised logsheet data (J. Hampton and Williams 2016; T.
Peatman, Williams, and Nicol 2021; T. Peatman, Williams, and Nicol
2023). For the Japanese (JP) fleet for which there is greater confidence
in species-based reporting, reported catch by species is used. Purse
seine catch for Philippines (PH) and Indonesian (ID) domestic purse
seine fisheries, predominantly operating in region 2, was derived from
raised port sampling data provided by these countries. We note that the
COVID-19 pandemic resulted in low observer coverage of the purse seine
fleets from mid 2020 to 2022 and purse seine catch estimates are less
reliable over these years.
...
Pacific-wide bigeye tuna stock assessment (S. McKechnie, Tremblay-Boyer,
and Harley 2015), and has subsequently been used in each bigeye and
yellowfin tuna assessment (S. McKechnie, Pilling, and Hampton 2017; M.
Vincent, Pilling, and Hampton 2018; Nicholas Ducharme-Barth et al. 2020;
Day et al. 2023). Since 2017 spatiotemporal approaches (James T. Thorson
et al. 2015) have been used for CPUE modeling in WCPFC stock assessments
(Tremblay-Boyer et al. 2017; N. Ducharme-Barth et al. 2020; T. Teears et
al. 2023). Consistent with the 2023 assessment, the CPUE modeling was
conducted with sdmTMB (Anderson et al. 2022-03). A detailed description
of the methods for generating the spatiotemporal abundance indices is
provided in T. Teears (2026). However, some aspects are worth noting
here. Firstly, the global analysis used for the regional scaling penalty
takes account of habitat viability by not including predictions in cells
when SST is below a specified thresholds. The threshold was determined
as 17°C based on the probability of positive CPUE observations being
greater than 10\% (T. Teears 2026). Applying the SST thresholds prevents
inflating the relative abundance in the northern and southern regions by
including spatial cells that have predictions of positive abundance that
are unlikely in reality. The approach was a recommendation from the PAW.
...
recommendations from PAW (P. Hamer 2026). The 2026 analysis implemented
...
assessment models. Please refer to T. Teears (2026) for a full
description of the CPUE modeling for generating the global and region
...
early 2000s for VN (Williams and Ruaia 2023; Vidal et al. 2026).
...
in P. Hamer et al. (2026). Note that these conversions occur in the
database prior to data extraction for stock assessments (P. Hamer et al.
...
P. Hamer et al. (2025); P. Hamer et al. (2026). Each size-frequency
record in the model consisted of the actual number of bigeye tuna
measured and Figure Figure~\ref{fig-data-coverage-tile} provides details
of the temporal availability of length and weight-frequency data (also
see T. Teears (2026)). Note that a maximum effective sample size of
1,000 is implemented in MFCL when using the robust normal likelihood for
size composition data. The effective sample size was further
down-weighted as explained in Section
Section~\ref{sec-sizefreq-likelihood}. Summaries of the available size
composition data by year and fishery are provided in T. Teears (2026)
and the processed data in T. Peatman, Kim, et al. (2026).
...
previous years (T. Teears et al. 2023; Day et al. 2023) and discussions
at the PAW (P. Hamer 2026), it was decided to include weight and length
data for longline fisheries due to the increasing coverage of length
data in recent years and the decreasing coverage of weight data (T.
Peatman, Kim, et al. 2026). This involved the conversion of weight data
to lengths external to the model, so that the final models ran entirely
on length data, this is the fist time to apply this approach. A key
principle used in constructing the size composition inputs was not to
use weight and length data at the same time for a fishery, even if it
was available. The decision was made to start replacing the weight data
(or weight converted to length) with actual length data if and when the
length data had greater coverage than the weight data. The preparation
of size data across the longline, purse seine, pole and line and other
domestic fisheries (Indonesia/Philippines/Vietnam), including the
integration of longline length data and the associated weight-length
conversion process is a significant amount of work, and is fully
described in the companion paper by T. Peatman, Kim, et al. (2026).
Preparation of size data inputs has increased substantially in recent
year, with increased review and scrutiny of these data and now take up a
more significant time component of data inputs preparation.
...
rate priors (from tag seeding experiments) can be found in T. Peatman,
Scutt Phillips, et al. (2026); T. Peatman et al. (2026); T. Teears
(2026). A summary of the tagging data and maps displaying tag
displacements are in T. Teears (2026). Data were available from the
Regional Tuna Tagging Project (RTTP) during 1989--92 (including
affiliated in-country projects in the Solomon Islands, Kiribati, Fiji
and the Philippines), the ongoing Pacific Tuna Tagging Programme (PTTP)
that began in 2006, see Figure Figure~\ref{fig-tag-release-recapt} and
Figure Figure~\ref{fig-tag-displacement-map}. The Japanese Tagging
Programme (JPTP) conducted by NRIFSF and the Ajinomoto Co.~Inc, over the
period 2000--2020 is also included. The new tagging data for the 2026
...
for the 2026 assessment (T. Teears 2026). Tags were released using
...
mortality. These adjustments are described in more detail in T. Peatman,
Scutt Phillips, et al. (2026); T. Teears (2026). An additional issue for
the bigeye assessment is that there are a considerable number of tag
returns that were released within the WCPO but recaptured to the east of
longitude 150\(^\circ\)W, outside the WCPO assessment region. The
adjustment or rescaling of releases for recaptures in the EPO preserves
the recovery rates of tags from individual tag groups that would
otherwise be biased low given that a considerable proportion of
recaptures cannot be attributed to a recapture category in the
assessment. These procedures were first described in Berger et al.
(2014-08) and S. McKechnie et al. (2016). However, the releases are not
adjusted for emigration outside of the model domain to the EPO. This is
an important issue that requires further consideration as to options for
informing adjustments to release numbers to account for emigration to
the EPO.
...
previous assessment based on the analysis of tag seeding experiments (T.
Peatman et al. 2026). Tag reporting rate groupings are required as not
all reporting fisheries have tag seeding experiments. These reporting
rate grouping decisions necessarily require some expert judgement. For
this assessment we have also excluded tag release groups with 5 or less
recaptures from the estimation of reporting rates, as we felt there was
insufficient information to inform model estimation of the reporting
rates. Tag reporting rate groupings are included in Table
...
are given in J. Hampton and Fournier (2001) and N. Davies et al.
(2026c), and summarised below. In addition, we describe the procedures
followed for estimating the parameters of the model and the way in which
stock status conclusions are drawn relative to a series of reference
points.
...
availability (K. M. Schaefer, Fuller, and Miyabe 2005; Sun, Chu, and Yeh
2006; Itano 2000). The assessment model assumed that recruitment occurs
...
spawning potential (Francis 1992; S. J. Harley 2011). Typically,
fisheries data are not very informative about the steepness parameter of
the SRR parameters (ISSF 2011); hence, the steepness parameter was fixed
at a moderate value (diagnostic model: 0.80) and the sensitivity of the
model results to the value of steepness was explored by setting it to
lower (0.65) and higher (0.95) values. The high CV (2.2) of the
...
parameters, as recommended by Ianelli, Maunder, and Punt (2012). The SRR
was estimated over the period 1962--2023 to prevent the earlier
recruitments (which may not be well estimated), and the terminal
recruitments (which are not freely estimated), from influencing the
relationship.
...
effect on recruitment and other model estimates (J. Hampton and Fournier
...
assessment review (Ianelli, Maunder, and Punt 2012).
...
weight-length relationship\footnote{The length-weight relationship has
  ...
  bias correction applied (P. Hamer et al. 2026)}. These processes are
assumed to be regionally and temporally invariant.

In previous assessments several approaches to growth estimation were
explored, including external otolith based growth curves (J. Farley et
al. 2020), a growth curve based on the otoliths with the addition of
...
(J. Farley et al. 2020), which also made use of the modal progressions
apparent in various size composition data (Nicholas Ducharme-Barth et
al. 2020). Results were sensitive to approaches used for growth. The
peer review of the yellowfin assessment (A. E. Punt, Maunder, and
Ianelli 2023) carefully considered the approaches to estimating growth.
They concluded that based on the otolith sampling protocols, with
otoliths selected according to length (i.e., attempting to achieve
sufficient numbers of otolith samples across the full length range, plus
bias in otolith readability with age), that any external growth curves
would likely be biased. They strongly recommended that for this type of
otolith sampling regime that age data should be conditional on length
and the growth estimated internally, using conditional-age-at-length
data within MFCL. This approach was applied in the 2023 assessment and
is continued for this assessment to estimate the von Bertalanffy growth
curve internal to the MFCL model, considering also information on growth
from size composition data. We also note the substantial enhancement of
the CAAL data set through the efforts of SPCs recently established
...
algorithm employed (see J. Hampton and Fournier (2001) and N. Davies et
al. (2026c) for details). Movement is parameterized by a pair of
...
in the 2020 assessments for both bigeye and yellowfin (Nicholas
Ducharme-Barth et al. 2020; Matthew Vincent et al. 2020) where
\(M\)-at-age was calculated using an approach applied to other tunas in
the WCPO and EPO (S. J. Harley and Maunder 2003; S. D. Hoyle 2008; S.
Hoyle and Nicol 2008). The yellowfin peer review had some difficulty
with tracing the sources of data for some of the inputs to the external
\(M\)-at-age function applied in 2020 that requires fitting a model that
depends on empirical data on the sex-ratio at length, a growth curve, a
base \(M\) assumption for males, assumptions on critical lengths for
inflections and a multiplier that determines the linear decline in \(M\)
for young ages to the base \(M\), plus a length at which female
mortality is assumed to begin to increase. There are uncertainties in
all these empirical data and assumptions, and the data for sex ratio is
limited especially for larger and older fish. While the peer review
generally supported the method, the construction of the external
\(M\)-at-age function, notably the limitations in some of the available
data required to estimate it, the complexity of the calculations and the
various assumption required are problematic. Furthermore, the external
\(M\)-at-age function requires re-estimation if different growth curves
are applied, further complicating use of this method. After the
yellowfin peer review, a Tuna Stock Assessment Good Practices workshop
run by the ``Center for the Advancement of Population Assessment
Methodology'' (CAPAM) was held (March 2023,
...
2015; André E. Punt et al. 2021-01). For the 2023 assessment M-at-age
was estimated internal to the model with a Lorenzen function (Day et al.
2023). However, the model was not stable to Jittering, and there were
concerns expressed by SC19 that instability may be partly related to the
internal estimation of both growth and M. Discussions at the 2026 PAW
recommended reverting back to fixing the adult/asymptotic M
(i.e.~M-scale, based an assumed M at age 40 quaters) initially at the
best estimate from the 2023 diagnostic model, with the Lorenzen M-at-age
shape being determined by the internally estimated growth curve
(inverse) (P. Hamer 2026).
...
previous, a companion paper estimated M based on tag-recapture data (N.
Ducharme-Barth et al. 2026), and the results of that study are included
in the M uncertainty range for this assessment.
...
potential-at-age using a smooth-spline approximation (N. Davies,
Fournier, and Hampton 2019). This allows for a more natural definition
of reproductive potential as the product of three length-based
processes: proportion females-at-length\footnote{For the current
  assessment, female sex-ratio-at-length was calculated from Regional
  Observer Program data in SPC's holdings through to 2018, consistent
  with the previous assessment as few data have been collected since.}
(sex-ratio), proportion of females mature-at-length\footnote{Taken from
  J. Farley et al. (2017) as in the previous assessment.}, and the
fecundity-at-length of mature females\footnote{Taken from Sun, Chu, and
  Yeh (2006) and standardized per kg of body weight at length as in the
  previous assessment.} (Figure Figure~\ref{fig-maturity-ogive}).
Another added benefit is that this reproductive potential ogive is
growth invariant. As in the 2023 assessment, we have not included
spawning fraction in the reproductive potential calculation, as these
data are still not adequate for bigeye in the WCPO.
...
yellowfin peer review (A. E. Punt, Maunder, and Ianelli 2023), have
developed an option to estimate length-based selectivity, which was
applied in the recent skipjack assessment (T. Teears et al. 2025). We
attempted the length-based selectivity option, however, despite various
configurations and model runs we could not achieve better/satisfactory
results in terms of data fits than the traditional age-based
selectivity. We ran out of time for deeper exploration and decided to
continue with age-based selectivity for the 2026 assessment.
...
reporting rates and their variances from tag seeding (T. Peatman et al.
2026). For the RTTP and PTTP, relatively informative ``priors'' were
...
bias estimation of management quantities (Kolody and Hoyle 2014). Mixing
...
recaptures per purse seine catch maps (i.e.~tag recapture rate maps) (T.
Teears et al. 2023). The 2023 yellowfin peer review noted the tag mixing
modeling approach recently being applied to skipjack (Castillo Jordan et
al. 2022; J. Scutt Phillips et al. 2022), wherebye an individual based
dispersal model is used to model dispersal and fishing mortality
exposure of tag release groups and contemporaneous untagged populations
as model particles. The approach simulates mixing periods specifically
for each release group, taking into account the unique locational and
temporal (environmental, fishing effort) contexts of each release event
constituting the group that may result in different rates of mixing of
released fish. It applies an individual based Lagrangian model
(Ikamoana)(J. Scutt Phillips et al. 2018) to track movement of
...
J. Scutt Phillips, Senina, and Bonin (2026). The peer review panel
recommended that this approach be explored for the yellowfin and bigeye
assessments.The approach was subsequently developed for this bigeye
assessment, and release group specific mixing periods were estimated, as
described below J. Scutt Phillips, Senina, and Bonin (2026).
...
releases were simulated from JPTP in region 1 (see (J. Scutt Phillips,
Senina, and Bonin 2026)). For release groups that were not directly
simulated, due to either computational reasons or a lack of forcing data
from SEAPODYM, a mixing period was assigned using averaged Kolmogorov
dissimilarity values from simulated groups. Specifically, for each
mixing scenario (zero, one, two and three quarters), the mean Kolmogorov
dissimilarity K was taken across the simulated release groups either
across all groups, or across all groups within a region from which an
indication of region-specific mixing was required, producing a single
representative K value per scenario. The cutoff selection rule was then
applied to this averaged set of four values exactly as it was for an
individual group: the assigned mixing period is the shortest scenario
whose mean K falls below the chosen cutoff, or four quarters if none
does. It is important to note that the averaging is performed on the
dissimilarity metric, at each mixing scenario, and not on the resulting
mixing periods themselves. Averaging the mixing periods would combine
values that are already the outcome of a threshold rule, discarding
information about how close each group was to its cutoff and giving a
non-integer result requiring arbitrary rounding; averaging the
underlying K values instead preserves the full recapture probability
distribution information from simulations, and applies the selection
...
from the standardisation models (T. Teears 2026) where each region was
...
Dirichlet-multinomial likelihood (J. T. Thorson et al. 2017). The
...
skipjack assessment (T. Teears et al. 2025), this effectively allows
...
similar to S. McKechnie, Pilling, and Hampton (2017), initial attempts
to estimate \(\tau\), assumed common for all fisheries and tag release
groups, tended to estimate a value close to 1, i.e.~Poisson, assumes
little or no over dispersion. Tag recapture data is likely to have a
degree of over dispersion due to clustering of recaptures, although it
likely depends on the level of dispersal and mixing after release.
Recaptures of tag groups shortly after release (that might fall in the
mixing period) could be expected to have higher overdispersion than
those recaptured after longer periods at liberty. Assigning a fixed
\(\tau\) to all fisheries and tag release groups is perhaps not ideal,
but a pragmatic approach given the limited time to statistically explore
the tag recapture overdispersion properties outside the model.
...
value of 2 as noted in those previous assessment reports (Nicholas
Ducharme-Barth et al. 2020; Day et al. 2023). This issue was not obvious
and required digging into MFCL code to discover. This meant that the
current diagnostic model that we had already fully diagnosed had a
\(\tau\) of 1.02. This is not desirable for tag recapture data, as noted
above. We therefore ran an additional model with \(\tau\) set to 2, as
this value appears to be considered suitable in previous assessment
methods, as it approaches a negative binomial with variance of twice the
Poisson, which can better account for overdispersion, but could
substantially downweight the tag data. This setting did allow a
comparison between a model that placed a high weight on the tag data
versus a model that downweighted the tag data. The value of \(\tau\) may
also interact with the mixing period assumptions in the uncertainty
ensemble, such that scenarios with longer tag mixing time, and therefore
less tags remaining in the joint likelihood estimation, would be subject
to less overdispersion, but be even more severely downweighted with an
increased \(\tau\). The use of \(\tau\), along with other assumptions
required for fitting tag recapture data in the joint likelihood requires
further technical review and guidance for future MFCL tuna assessments.
...
(A. E. Punt, Maunder, and Ianelli 2023). These data are included in the
assessment and contribute to estimation of the growth parameters, as
they provide direct observations of the distribution of ages, generally
measured with some error, within each observed length class. For each
fishery and time period (quarter) aged fish are separated into observed
(measured) length classes, with the distribution of ages tabulated as a
function of the known length (ideally with ageing measurement error, but
this feature is still to be fully developed and tested in MFCL). The
measured age distribution within each length interval is assumed to be
multinomially distributed, and this forms the basis of the likelihood
component for these data. However, despite the otolith data being
collected across a range of locations and times the
conditional-age-at-length data generally have a degree of
non-independence, as is the case for size compositional data, so these
conditional age-at-length data were downweighted by a factor of 0.75 for
...
MFCL for implementing these weights (scaling factors) (N. Davies et al.
2026b), which has the effect of adjusting (scaling) the effective
...
model parameters (auto-differentiation, D. A. Fournier et al. (2012)).
...
the terminal quarters sequentially excluded (Cadigan and Farrell 2005).
...
scaling, following the procedure outlined by S. McKechnie, Pilling, and
Hampton (2017) and Tremblay-Boyer et al. (2017). The results of these
procedures are presented for the diagnostic model Section
...
methods involved are summarized below and the details can be found in N.
Davies et al. (2026c).
...
in N. Davies et al. (2026c), and is equivalent to the MSY. Similarly,
the spawning potential at MSY \(SB_{MSY}\) can be determined from this
...
\item
  ...
  where their catch coverage exceeded that of weight samples (T.
  Peatman, Kim, et al. 2026)
\item
...
\item
  ...
  yellowfin assessment peer review (A. E. Punt, Maunder, and Ianelli
  2023; P. Hamer 2026).
\item
...
\item
  ...
  (see appendix Tables in J. Scutt Phillips, Senina, and Bonin (2026)
\item
...
\item
  ...
  0.062/quarter (N. Ducharme-Barth et al. 2026), Higher M = 0.10/quarter
  (diagnostic 2023: 0.078/quarter)
\item
  Effort creep on longline CPUE indices: `maximum' scenario suggested
  from PAW (S. Hoyle 2024) 3\% per year pre 1976, 1.5\% year post 1976,
...
values and the assigned mixing periods can be viewed in J. Scutt
Phillips, Senina, and Bonin (2026). Finally, the sensitivity analysis
showed that once \(\tau\) increased beyond 1.4 the key results were
insensitive, so \(\tau\) above 1.4 seems somewhat redundant in further
downweighting the tag data influence. However there was sensitivity
between 1 and 1.4. Acknowledging that the basis for setting a \(\tau\)
value is not clear, other than it should be above 1, we include three
values for \(\tau\) in the uncertainty ensemble: 1.2, 1.3, 1.4.
...
Monte Carlo ensemble approach applied in the most recent skipjack (T.
Teears et al. 2025) and south Pacific albacore assessments (T. Teears et
...
\href{https://github.com/PacificCommunity/ofp-sam-bet-2026-stepwise/releases/latest/download/interactive-model-viewer.html}{interactive
HTML viewer}.
...
\href{https://github.com/PacificCommunity/ofp-sam-bet-2026-sensitivity/releases/latest/download/bet-2026-sensitivity-interactive-viewer.html}{interactive
...
estimate and 90\% confidence interval from N. Ducharme-Barth et al.
(2026).}
...
Aires-da-Silva, A M, M N Maunder, K M Schaefer, and D W Fuller. 2015.
{``Improved Growth Estimates from Integrated Analysis of Direct Aging
and Tag-Recapture Data: An Illustration with Bigeye Tuna (Thunnus
Obesus) of the Eastern Pacific Ocean with Implications for
Management.''} \emph{Fisheries Research} 163: 119--26.

\bibitem[\citeproctext]{ref-anderson_sdmtmb_2022}
Anderson, Sean C., Eric J. Ward, Philina A. English, and Lewis A. K.
Barnett. 2022-03. {``{sdmTMB}: An r Package for Fast, Flexible, and
...
Berger, A M, S McKechnie, F Abascal, B Kumasi, T Usu, and S J Nichol.
2014-08. {``Analysis of Tagging Data for the 2014 Tropical Tuna
Assessments: Data Quality Rules, Tagger Effects, and Reporting Rates.''}
{WCPFC}-{SC}10-2014/{SA}-{IP}-06. Majuro, Republic of the Marshall
Islands, 6-14 August 2014.: WCPFC.

\bibitem[\citeproctext]{ref-cadigan_local_2005}
Cadigan, Noel G., and Patrick J. Farrell. 2005. {``Local Influence
Diagnostics for the Retrospective Problem in Sequential Population
Analysis.''} \emph{{ICES} Journal of Marine Science: Journal Du Conseil}
62 (2): 256--65. \url{https://doi.org/10.1016/j.icesjms.2004.11.015}.

\bibitem[\citeproctext]{ref-castillo_jordan_stock_2022}
Castillo Jordan, C, T Teears, J Hampton, N. Davies, J. Scutt Phillips,
S. McKechnie, T. Peatman, et al. 2022. {``Stock Assessment of Skipjack
Tuna in the Western and Central Pacific Ocean: 2022.''}
{WCPFC}-{SC}18-{SA}-{WP}-01. WCPFC.

\bibitem[\citeproctext]{ref-chow_genetic_2000}
Chow, Seinen, Hiroaki Okamoto, Naozumi Miyabe, Kazuhiko Hiramatsu, and
Noel Barut. 2000. {``Genetic Divergence Between Atlantic and
Indo-Pacific Stocks of Bigeye Tuna (Thunnus Obesus) and Admixture Around
South Africa.''} \emph{Molecular Ecology} 9 (2): 221--27.
\url{https://doi.org/10.1046/j.1365-294x.2000.00851.x}.

\bibitem[\citeproctext]{ref-davies_developments_2026-1}
Davies, N., F. Bouye, K Kim, A. Magnusson, and J. Hampton. 2026a.
{``Developments in the {MULTIFAN}-{CL} Software 2025-26.''}
{WCPFC}-{SC}22-2026/{SA}-{IP}-02. WCPFC.

\bibitem[\citeproctext]{ref-davies_developments_2026}
Davies, N., F Bouye, K Kim, A Magnusson, and J. Hampton. 2026b.
{``Developments in the {MULTIFAN}-{CL} Software 2025-26.''}
{WCPFC}-{SC}22-2026/{SA}-{IP}-02. WCPFC.

\bibitem[\citeproctext]{ref-davies_multifan-cl_2026}
Davies, N, D A Fournier, J. Hampton, P. Kleiber, F. Bouye, K Kim, A.
Magnusson, and S. Hoyle. 2026c. {``{MULTIFAN}-{CL} User's Guide Version
2.2.7.8.''} Noumea, New Caledonia: WCPFC.

\bibitem[\citeproctext]{ref-davies_developments_2019}
Davies, N, D Fournier, and J Hampton. 2019. {``Developments in the
{MULTIFAN}-{CL} Software 2018-2019.''} {WCPFC}-{SC}15-2019/{SA}-{IP}-02.
Pohnpei, Federated States of Micronesia: WCPFC.

\bibitem[\citeproctext]{ref-davies_stock_2011}
Davies, N, S Hoyle, S Harley, A Langley, P Kleiber, and J Hampton. 2011.
{``Stock Assessment of Bigeye Tuna in the Western and Central Pacific
Ocean.''} {WCPFC}-{SC}7-2011/{SA}-{WP}-02. Pohnpei, Federated States of
Micronesia: WCPFC.

\bibitem[\citeproctext]{ref-day_stock_2023}
Day, J, A. Magnusson, T. Teears, J. Hampton, N. Davies, C Castillo
Jordán, T. Peatman, et al. 2023. {``Stock Assessment of Bigeye Tuna in
the Western and Central Pacific Ocean: 2023.''}
{WCPFC}-{SC}19-2023/{SA}-{WP}-05 (Rev. 2). WCPFC.

\bibitem[\citeproctext]{ref-ducharme-barth_stock_2020}
Ducharme-Barth, Nicholas, Matthew Vincent, John Hampton, Paul Hamer,
Peter Williams, and Graham Pilling. 2020. {``Stock Assessment of Bigeye
Tuna in the Western and Central Pacific Ocean.''}
{WCPFC}-{SC}16-2020/{SA}-{WP}-03 {[}{REV}3{]}. WCPFC.

\bibitem[\citeproctext]{ref-ducharme-barth_natural_2026}
Ducharme-Barth, N., T. Peatman, J. Scutt Phillips, and C. Minte-Vera.
2026. {``Natural Mortality Estimation for {WCPO} Bigeye Tuna: A Joint
Cohort Analysis of Coral Sea and Region 4 Tagging Data.''}
{WCPFC}-{SC}22-2026/{SA}-{IP}-14. WCPFC.

\bibitem[\citeproctext]{ref-ducharme-barth_analysis_2020}
Ducharme-Barth, N, M Vincent, T Vidal, and P Hamer. 2020. {``Analysis of
Pacific-Wide Operational Longline Dataset for Bigeye and Yellowfin Tuna
Catch-Per-Unit-Effort ({CPUE}).''} {SC}16-{SA}-{IP}-07. WCPFC.

\bibitem[\citeproctext]{ref-evans_behaviour_2008}
Evans, K., A. Langley, N. P. Clear, P. Williams, T. Patterson, J.
Sibert, J. Hampton, and J. S. Gunn. 2008. {``Behaviour and Habitat
...
Journal of Fisheries and Aquatic Sciences} 65: 2427--43.

\bibitem[\citeproctext]{ref-eveson_integrated_2020}
Eveson, P, M Vincent, J Farley, K Krusic-Golub, and J Hampton. 2020.
...
Farley, J., P. Eveson, K. Krusic-Golub, C Sanchez, F Rouspard, S
McKechnie, S Nichol, B Leroy, N Smith, and S-K Chang. 2017. {``Age,
Growth and Maturity of Bigeye Tuna in the Pacific.''} {WCPFC}-{SC}13-
2017/{SA}-{WP}-01. Rarotonga, Cook Islands, 9-17 August 2017: WCPFC.

\bibitem[\citeproctext]{ref-farley_age_2020}
Farley, J, K Krusic-Golub, P Eveson, N Clear, F Rouspard, C Sanchez, S
Nicol, and J Hampton. 2020. {``Age and Growth of Yellowfin and Bigeye
Tuna in the Western and Central Pacific Ocean from Otoliths.''}
{SC}16-{SA}-{WP}-02. WCPFC.

\bibitem[\citeproctext]{ref-fournier_ad_2012}
Fournier, D A, H J Skaug, J Ancheta, J. Ianelli, A Magnusson, M N
Maunder, A Nielson, and J. Sibert. 2012. {``{AD} Model Builder: Using
Automatic Differentiation for Statistical Inference of Highly
Parameterized Complex Nonlinear Models.''} \emph{Optimization Methods
and Software} 27 (2): 233--49.

\bibitem[\citeproctext]{ref-fournier_multifan-cl_1998}
Fournier, D. A., J. Hampton, and J. R. Sibert. 1998. {``{MULTIFAN}-{CL}:
...
\emph{Canadian Journal of Fisheries and Aquatic Sciences} 55: 2105--16.
...
Aquatic Sciences} 39 (8): 1195--1207.
...
Management Strategies: A Case Study Using Orange Roughy (It Hoplostethus
Atlanticus) on the Chatham Rise, New Zealand.''} \emph{Canadian Journal
of Fisheries and Aquatic Science} 49: 922--30.

\bibitem[\citeproctext]{ref-francis_data_2011}
---------. 2011. {``Data Weighting in Statistical Fisheries Stock
Assessment Models.''} \emph{Canadian Journal of Fisheries and Aquatic
Sciences} 68: 1124--38.

\bibitem[\citeproctext]{ref-gonzalez_genetic_2008}
Gonzalez, Elena G, Peter Beerli, and Rafael Zardoya. 2008. {``Genetic
Structuring and Migration Patterns of Atlantic Bigeye Tuna, Thunnus
Obesus (Lowe, 1839).''} \emph{{BMC} Evolutionary Biology} 8 (1): 252.
\url{https://doi.org/10.1186/1471-2148-8-252}.

\bibitem[\citeproctext]{ref-grewe_assessment_1998}
Grewe, P. M., and J. Hampton. 1998. {``An Assessment of Bigeye (Thunnus
Obesus) Population Structure in the Pacific Ocean Based on Mitochondrial
{DNA} and {DNA} Microsatellite Analysis.''} {JIMAR} Contribution 98-330.

\bibitem[\citeproctext]{ref-grewe_population_2019}
Grewe, P. M., C. H. Wudianto, C. H. Proctor, M. S. Adam, A. R. Jauhary,
K Schaefer, D. G. Itano, et al. 2019. {``Population Structure and
Connectivity of Tropical Tuna Species Across the Indo Pacific Ocean
Region.''} {WCPFC}-{SC}15-2019/{SA}-{IP}-15. WCPFC.
...
- March 2026.''} {WCPFC}-{SC}22-2026/{SA}-{IP}-01. WCPFC.

\bibitem[\citeproctext]{ref-hamer_wcpfc_2026}
Hamer, P., C Castillo Jordán, A. Panizza, and T. Peatman. 2026.
{``{WCPFC} Project127/127a - Review and Reconciliation of Size Data
...
Hamer, P, J Potts, J Macdonald, and I Senina. 2023. {``Review and
Analyses to Inform Conceptual Models of Population Structure and Spatial
Stratification of Bigeye and Yellowfin Tuna Assessments in the Western
and Central Pacific Ocean.''} {SC}19-{SA}-{WP}-02. Palau: WCPFC.

\bibitem[\citeproctext]{ref-hamer_progress_2025}
Hamer, P, E Schneiter, T Vidal, and P Williams. 2025. {``Progress
Report: Review and Reconciliation of Size Data Collected in the
{WCPFC}-{CA} for Stock Assessment Purposes ({WCPFC} Project: 127).''}
{WCPFC}-{SC}21-{ST}-{WP}-02. WCPFC.
...
Hampton, J., N. Davies, and P. Hamer. 2026. {``Investigation of the
Impacts of Regional Growth Variation on the 2023 Bigeye Tuna Stock
Assessment.''} {WCPFC}-{SC}22-2026/{SA}-{IP}11. WCPFC.

\bibitem[\citeproctext]{ref-hampton_spatially-disaggregated_2001}
Hampton, J., and D. A. Fournier. 2001. {``A Spatially-Disaggregated,
...
Freshwater Research} 52: 937--63.

\bibitem[\citeproctext]{ref-hampton_investigation_2026}
Hampton, J., T Teears, K Kim, A. Magnusson, N. Davies, and P. Hamer.
2026. {``Investigation of Single Region Fleets as Areas Models for
Bigeye and Yellowfin Tuna in the {WCPO}.''}
{WCPFC}-{SC}22-2026/{SA}-{IP}-09. WCPFC.

\bibitem[\citeproctext]{ref-hampton_annual_2016}
Hampton, J, and P Williams. 2016. {``Annual Estimates of Purse Seine
Catches by Species Based on Alternative Data Sources.''}
{WCPFC}-{SC}12-2016/{ST}-{IP}-03. Bali, Indonesia, 3-11 August 2016:
WCPFC.

\bibitem[\citeproctext]{ref-hare_western_2025}
Hare, S., T. Vidal, C Castillo Jordán, J. Day, P. Hamer, N. Hill, P.
Machful, et al. 2025. {``The Western and Central Pacific Una Fishery:
2024 Overview and Status of Stocks.''} Tuna Fisheries Assessment Report
No. 25. Noumea, New Caledonia: SPC.

\bibitem[\citeproctext]{ref-hare_compendium_2026}
Hare, S., T. Vidal, R Scott, and G Pilling. 2026. {``A Compendium of
Fisheries Indicators for Target Tuna Stocks in the {WCPFC} Convention
Area.''} {WCPFC}-{SC}22-2026/{SA}-{WP}-07. WCPFC.

\bibitem[\citeproctext]{ref-harley_simple_2003}
Harley, S J, and M N Maunder. 2003. {``A Simple Model for Age-Structured
Natural Mortality Based on Changes in Sex Ratios.''} {SAR}-4-01. La
Jolla, California, {USA}, 19-21 May 2003: Inter-American Tropical Tuna
Commission.

\bibitem[\citeproctext]{ref-harley_preliminary_2011}
Harley, S. J. 2011. {``A Preliminary Investigation of Steepness in Tunas
Based on Stock Assessment Results.''} {WCPFC}-{SC}7-2011/{SA}-{IP}-08.
Pohnpei, Federated States of Micronesia, 9-17 August 2011.: WCPFC.

\bibitem[\citeproctext]{ref-harley_stock_2010}
Harley, S. J., S. Hoyle, J. Hampton, and P. Kleiber. 2010. {``Stock
Assessment of Bigeye Tuna in the Western and Central Pacific Ocean.''}
{WCPFC}-{SC}6-2010/{SA}-{WP}-01. Nuku'alofa, Tonga, 10-19 August 2010.:
WCPFC.

\bibitem[\citeproctext]{ref-harley_stock_2014}
Harley, Shelton J, Nick Davies, John Hampton, and Sam McKechnie.
2014-08. {``Stock Assessment of Bigeye Tuna in the Western and Central
Pacific Ocean.''} {WCPFC}-{SC}10-2014/{SA}-{WP}-01. Majuro, Republic of
the Marshall Islands, 6-14 August 2014.: WCPFC.
\url{http://wcpfc.int/system/files/SC10-SA-WP-01\%20BET\%20Assessment.pdf}.

\bibitem[\citeproctext]{ref-harley_stock_2009}
Harley, S, S Hoyle, and A Langley. 2009. {``Stock Assessment of Bigeye
Tuna in the Western and Central Pacific Ocean.''}
{WCPFC}-{SC}5-2009/{SA}-{WP}-04. Port Vila, Vanuatu, 10-21 August 2009:
WCPFC.

\bibitem[\citeproctext]{ref-hasegawa_new_2026}
Hasegawa, T., K Okamoto, H Ijima, K Kim, P. Hamer, C Wakefield, A Pons,
J. Widdrington, and N Davies. 2026. {``New Age-at-Length Data for {WCPO}
Bigeye and Yellowfin Tuna from the Temperate North Pacific and
Approaches to Address Spatial Sampling Bias in Growth Estimation Within
{MFCL}.''} {WCPFC}-{SC}22-2026/{IP}-08. WCPFC.

\bibitem[\citeproctext]{ref-hoyle_effort_2024}
Hoyle, S. 2024. {``Effort Creep in Longline and Purse Seine {CPUE} and
Its Application in Tropical Tuna Assessments.''}
{WCPFC}-{SC}20-2024/{SA}-{IP}-09. WCPFC.

\bibitem[\citeproctext]{ref-hoyle_adjusted_2008}
Hoyle, S. D. 2008. {``Adjusted Biological Parameters and Spawning
...
Port Moresby, Papua New Guinea, 11-22 August 2008.: WCPFC.

\bibitem[\citeproctext]{ref-hoyle_sensitivity_2008}
Hoyle, S., and S. Nicol. 2008. {``Sensitivity of Bigeye Stock Assessment
to Alternative Biological and Reporductive Assumptions.''}
{WCPFC}-{SC}4-2008/{ME}-{WP}-01. Port Moresby, Papua New Guinea, 11-22
August 2008.: WCPFC.

\bibitem[\citeproctext]{ref-hurtado-ferro_looking_2015}
Hurtado-Ferro, Felipe, Cody S. Szuwalski, Juan L. Valero, Sean C.
Anderson, Curry J. Cunningham, Kelli F. Johnson, Roberto Licandeo, et
al. 2015. {``Looking in the Rear-View Mirror: Bias and Retrospective
Patterns in Integrated, Age-Structured Stock Assessment Models.''}
\emph{Ices Journal of Marine Science} 72 (1): 99--110.
\url{https://doi.org/10.1093/icesjms/fsu198}.

\bibitem[\citeproctext]{ref-ianelli_independent_2012}
Ianelli, Jim, Mark N. Maunder, and Andre. E. Punt. 2012. {``Independent
Review of the 2011 {WCPO} Bigeye Tuna Assessment.''}
{WCPFC}-{SC}8-2012/{SA}-{WP}-01. Busan, Republic of Korea, 7-15 August
2012: WCPFC.

\bibitem[\citeproctext]{ref-ijima_tuna_2023}
Ijima, Hirotaka, and Marko Jusup. 2023-04. {``Tuna and Billfish Larval
Distributions in a Warming Ocean.''} {arXiv}.
\url{http://arxiv.org/abs/2304.09442}.

\bibitem[\citeproctext]{ref-issf_report_2011}
ISSF. 2011. {``Report of the 2011 {ISSF} Stock Assessment Workshop.''}
{ISSF} Technical Report 2011-02. Rome, Italy, March 14-17: ISSF.

\bibitem[\citeproctext]{ref-itano_reproductive_2000}
Itano, D. 2000. {``The Reproductive Biology of Yellowfin Tuna (It
Thunnus Albacares) in Hawaiian Waters and the Western Tropical Pacific
Ocean: Project Summary.''} \{JIMAR\} Contribution 00-328 {SOEST} 00-01.
University of Hawai'i.

\bibitem[\citeproctext]{ref-kolody_evaluation_2014}
Kolody, D., and S. Hoyle. 2014. {``Evaluation of Tag Mixing Assumptions
in Western Pacific Ocean Skipjack Tuna Stock Assessment Models.''}
\emph{Fisheries Research}
http://dx.doi.org/101016/j.fishres.2014.05.008.
...
McKechnie, Sam, L Tremblay-Boyer, and G Pilling. 2017. {``Background
Analyses for the 2017 Stock Assessments of Bigeye and Yellowfin Tuna in
the Western and Central Pacific Ocean.''}
{WCPFC}-{SC}13-2017/{SA}-{IP}-06. Rarotonga, Cook Islands: WCPFC.

\bibitem[\citeproctext]{ref-mckechnie_sensitivity_2015}
McKechnie, S., J. Hampton, Francisco Abascal, N. Davies, and S. J.
Harley. 2015. {``Sensitivity of {WCPO} Stock Assessment Results to the
Inclusion of {EPO} Dynamics Within a Pacific-Wide Analysis.''}
{WCPFC}-{SC}11-2015/{SA}-{WP}-04. Pohnpei, Federated States of
Micronesia, 5-13 August 2015: WCPFC.

\bibitem[\citeproctext]{ref-mckechnie_construction_2016}
McKechnie, S., D. Ochi, H. Kiyofuji, T. Peatman, and S. Caillot. 2016.
{``Construction of Tagging Data Input Files for the 2016 Skipjack Tuna
Stock Assessment in the Western and Central Pacific Ocean.''}
{WCPFC}-{SC}12-2016/{SA}-{IP}-05. Bali, Indonesia, 3-11 August 2016:
WCPFC.

\bibitem[\citeproctext]{ref-mckechnie_stock_2017}
McKechnie, S., G Pilling, and J. Hampton. 2017. {``Stock Assessment of
Bigeye Tuna in the Western and Central Pacific Ocean.''}
{WCPFC}-{SC}13-2017/{SA}-{WP}-05. Rarotonga, Cook Islands, 9-17 August
2017: WCPFC.

\bibitem[\citeproctext]{ref-mckechnie_summaries_2026}
McKechnie, S, G Pilling, and T. Vidal. 2026. {``Summaries of Bigeye Tuna
Catch and Effort to Support Management Procedure Discussions.''}
{WCPFC}-{SC}22-2026/{SA}-{IP}-12. WCPFC.

\bibitem[\citeproctext]{ref-mckechnie_analysis_2015}
McKechnie, S., L Tremblay-Boyer, and S J Harley. 2015. {``Analysis of
Pacific-Wide Operational Longline {CPUE} Data for Bigeye Tuna.''}
{WCPFC}-{SC}11-2015/{SA}-{WP}-03. Pohnpei, Federated States of
Micronesia, 5-13 August 2015: WCPFC.
...
\emph{Ices Journal of Marine Science} 56 (4): 473--88.
\url{https://doi.org/10.1006/jmsc.1999.0481}.

\bibitem[\citeproctext]{ref-moore_defining_2020}
Moore, Bradley R., Johann D. Bell, Karen Evans, Jessica Farley, Peter M.
Grewe, John Hampton, Amandine D. Marie, et al. 2020-10. {``Defining the
Stock Structures of Key Commercial Tunas in the Pacific Ocean i: Current
Knowledge and Main Uncertainties.''} \emph{Fisheries Research} 230
(2020-10): 105525. \url{https://doi.org/10.1016/j.fishres.2020.105525}.

\bibitem[\citeproctext]{ref-moore_movement_2019}
Moore, Bradley R, Pratiwi Lestari, Scott C Cutmore, Craig Proctor, and
Robert J G Lester. 2019-12. {``Movement of Juvenile Tuna Deduced from
...
Natasha, Janice, Brian L. Stockwell, Amandine D. Marie, John Hampton,
Neville Smith, Simon Nicol, and Ciro Rico. 2022-04. {``No Population
Structure of Bigeye Tunas (Thunnus Obesus) in the Western and Central
Pacific Ocean Indicated by Single Nucleotide Polymorphisms.''}
\emph{Front. Mar. Sci.} 9 (2022-04): 799684.
\url{https://doi.org/10.3389/fmars.2022.799684}.

\bibitem[\citeproctext]{ref-nicol_bigeye_2011}
Nicol, S., S. Hoyle, J. Farley, B. Muller, S. Retalmai, K. Sisior, and
A. Williams. 2011. {``Bigeye Tuna Age, Growth, and Reproductive Biology
(Project 35).''} {WCPFC}-{SC}7-2011/{SA}-{WP}-01. Pohnpei, Federated
States of Micronesia, 9-17 August 2011.: WCPFC.

\bibitem[\citeproctext]{ref-nishikawa_average_1985}
Nishikawa, Y, M Honma, S Ueyanagi, and S Kikawa. 1985. {``Average
...
Peatman, T, S Fukofuka, T Park, P Williams, J Hampton, and N Smith.
2019. {``Better Purse Seine Catch Composition Estimates: Progress on the
Project 60 Work Plan.''} Technical Report. SPC.

\bibitem[\citeproctext]{ref-peatman_analysis_2026-2}
Peatman, T, S. Gislard, J. Scutt Phillips, J. Potts, and S. Nicol. 2026.
{``Analysis of Tag Seeding Data for the 2026 Yellowfin \& Bigeye
...
Peatman, T., K Kim, A. Magnusson, C Castillo Jordan, and P. Hamer. 2026.
{``Analysis of Size Frequency Data for the 2026 Bigeye and Yellowfin
Assessments.''} {WCPFC}-{SC}22-{SA}-{IP}06. WCPFC.

\bibitem[\citeproctext]{ref-peatman_analysis_2026-1}
Peatman, T., J. Scutt Phillips, J. Potts, and S Nicol. 2026. {``Analysis
of Tagging Data for the 2026 Yellowfin \& Bigeye Assessments:
Corrections to Tag Releases for Tagging Conditions.''}
{WCPFC}-{SC}22-2026/{SA}-{IP}-04. WCPFC.

\bibitem[\citeproctext]{ref-peatman_project_2023}
Peatman, T., P. Williams, and S. Nicol. 2023. {``Project 60: {Progress}
Report - Improving Purse Seine Species Composition.''} \{WCPFC\}
\{Scientific\} \{Committee\} paper SC19-ST-IP-03. Palau.

\bibitem[\citeproctext]{ref-peatman_project_2021}
Peatman, T, P. Williams, and S Nicol. 2021. {``Project 60: Progress
Towards Acheiving {SC}16 Recomendations.''}
{WCPFC}-{SC}17-2021/{ST}-{IP}-04. WCPFC.

\bibitem[\citeproctext]{ref-pilling_approaches_2016}
Pilling, G, R Scott, N Davies, and J Hampton. 2016. {``Approaches Used
to Undertake Management Projections of {WCPO} Tuna Stocks Based Upon
{MULTIFAN}-{CL} Stock Assessments.''} {WCPFC}-{SC}12-2016/{MI}-{IP}-04.
Bali, Indonesia, 3-11 August 2016.

\bibitem[\citeproctext]{ref-piner_meta-analysis_2011}
Piner, K, and H Lee. 2011. {``Meta-Analysis of Striped Marlin Natural
Mortality.''} {ISC}/11/{BILLWG}‐1/10. ISC.

\bibitem[\citeproctext]{ref-proctor_population_2019}
Proctor, C. H., R. G. J. Lester, N. P. Clear, P. M. Grewe, Bradley R.
Moore, J. P. Eveson, P Lestari, et al. 2019. {``Population Structure of
Yellowfin Tuna (Thunnus Albacares) and Bigeye Tuna (t. Obesus) in the
Indonesian Region.''} Final Report as output of {ACIAR} Project
{FIS}/2009/059. Australian Centre for International Agricultural
Research, Canberra: ACIAR.

\bibitem[\citeproctext]{ref-punt_independent_2023}
Punt, A. E., M N Maunder, and J. Ianelli. 2023. {``Independent Review of
Recent {WCPO} Yellowfin Tuna Assessment.''} {SC}19-{SA}-{WP}-01. WCPFC.

\bibitem[\citeproctext]{ref-punt_consequences_2021}
Punt, André E., Claudio Castillo-Jordán, Owen S. Hamel, Jason M. Cope,
Mark N. Maunder, and James N. Ianelli. 2021-01. {``Consequences of Error
in Natural Mortality and Its Estimation in Stock Assessment Models.''}
\emph{Fisheries Research} 233 (2021-01): 105759.
\url{https://doi.org/10.1016/j.fishres.2020.105759}.

\bibitem[\citeproctext]{ref-rooker_natal_2016}
Rooker, Jay R., R. J. David Wells, David G. Itano, Simon R. Thorrold,
and Jessica M. Lee. 2016-05. {``Natal Origin and Population Connectivity
of Bigeye and Yellowfin Tuna in the Pacific Ocean.''} \emph{Fish.
Oceanogr.} 25 (3): 277--91. \url{https://doi.org/10.1111/fog.12154}.

\bibitem[\citeproctext]{ref-schaefer_reproductive_2005}
Schaefer, K. M., D. W. Fuller, and N. Miyabe. 2005. {``Reproductive
Biology of Bigeye Tuna (It Thunnus Obesus) in the Eastern and Central
Pacific Ocean.''} 23(1). 35p. {IATTC} Bulletin.

\bibitem[\citeproctext]{ref-schaefer_movements_2015}
Schaefer, K., D. Fuller, J. Hampton, S. Caillot, B. Leroy, and D. Itano.
2015. {``Movements, Dispersion, and Mixing of Bigeye Tuna (It Thunnus
Obesus) Tagged and Released in the Equatorial Central Pacific Ocean,
with Conventional and Archival Tags.''} \emph{Fisheries Research} 161:
336--55.

\bibitem[\citeproctext]{ref-scutt_phillips_quantifying_2022}
Scutt Phillips, J, J Lehodey, J Hampton, I Senina, and S Nicol. 2022.
{``Quantifying Rates of Mixing in Tagged, {WCPO} Skipjack Tuna.''}
{WCPFC}-{SC}18-2022/{SA}-{WP}-04. WCPFC.

\bibitem[\citeproctext]{ref-scutt_phillips_individual-based_2018}
Scutt Phillips, J., A. Sen Gupta, I. Senina, E. Sebille, M. Lange, P.
Lehodey, Hampton J, and S. Nichol. 2018. {``An Individual-Based Model of
Skipjack Tuna Katsuwonus Pelamis Movement in the Tropical Pacific
Ocean.''} \emph{Progress in Oceanography} 164: 63--74.

\bibitem[\citeproctext]{ref-scutt_phillips_estimation_2026}
Scutt Phillips, J., I. Senina, and L. Bonin. 2026. {``Estimation of Tag
Mixing Periods for Teh 2026 {WCPO} Tuna Stock Assessments.''}
{WCPFC}-{SC}22-2026-{SA}-{IP}-10. WCPFC.

\bibitem[\citeproctext]{ref-senina_seapodym_2020}
Senina, I, P. Lehodey, S Nicol, J Scutt Phillips, and J Hampton. 2020.
{``{SEAPODYM}: Revisiting Bigeye Reference Model with Conventional
Tagging Data.''} {WCPFC}-{SC}16-2020/{EB}-{IP}-05. WCPFC.

\bibitem[\citeproctext]{ref-sun_reproductive_2006}
Sun, C-L., S-L. Chu, and S-Z. Yeh. 2006. {``Reproductive Biology of
Bigeye Tuna in the Western and Central Pacific Ocean.''}
{WCPFC}-{SC}2-2006/{BI}-{WP}-01. Manila, Philippines, 7-18 August 2006:
WCPFC.

\bibitem[\citeproctext]{ref-teears_background_2026}
Teears, T. 2026. {``Background Analyses and Data Inputs for the 2026
{WCPO} Yellowfin and Bigeye Tuna Stock Assessment.''}
{WCPFC}-{SC}22-2026-{SA}-{IP}03. WCPFC.

\bibitem[\citeproctext]{ref-teears_stock_2024}
Teears, T., C Castillo Jordán, N Davies, J. Day, J. Hampton, A.
Magnusson, T. Peatman, et al. 2024. {``Stock Assessment of South Pacific
Albacore: 2024.''} {WCPFC}-{SC}20-2024/{SA}-{WP}-02-Rev 2. WCPFC.

\bibitem[\citeproctext]{ref-teears_stock_2025}
Teears, T, N Davies, P Hamer, A Magnusson, T Peatman, J Scutt Phillips,
T Vidal, and J Hampton. 2025. {``Stock Assessment of Skipjack Tuna in
the Western and Central Pacific Ocean: 2025 (Rev 3).''}
{WCPFC}-{SC}21-2025/{SA}-{WP}-02\_rev\_3. WCPFC.
\url{https://meetings.wcpfc.int/node/26679}.

\bibitem[\citeproctext]{ref-teears_cpue_2023}
Teears, T., J. Day, J. Hampton, A. Magnusson, S. McKechnie, T. Peatman,
J. Scutt Phillips, P. Williams, and P. Hamer. 2023. {``{CPUE} Analysis
and Data Inputs for the 2023 Bigeye and Yellowfin Tuna Assessments in
the {WCPO}.''} {WCPFC}-{SC}19-2023/{SA}-{WP}-03. WCPFC.

\bibitem[\citeproctext]{ref-thorson_model-based_2017}
Thorson, J. T., K. F. Johnson, R. D. Methot, and I. G. Taylor. 2017.
...
Thorson, James T., Andrew O. Shelton, Eric J. Ward, and Hans J. Skaug.
2015. {``Geostatistical Delta-Generalized Linear Mixed Models Improve
...
Tremblay-Boyer, L, S McKechnie, G Pilling, and J Hampton. 2017. {``Stock
Assessment of Yellowfin Tuna in the Western and Central Pacific
Ocean.''} {WCPFC}-{SC}13-2017/{SA}-{WP}-06. Rarotonga, Cook Islands,
9-17 August 2017: WCPFC.

\bibitem[\citeproctext]{ref-vidal_data_2026}
Vidal, T., F. Heather, F Satria, V. Hai, S Barcoma, N Duong, L Sadiyah,
et al. 2026. {``Data Improvements in the West Pacific East Asia ({WPEA})
Region of {WCPFC}.''} {WCPFC}-{SC}22-2026/{ST}-{IP}-05. WCPFC.

\bibitem[\citeproctext]{ref-vincent_stock_2020}
Vincent, Matthew, Nicholas Ducharme-Barth, Paul Hamer, John Hampton,
Peter Williams, and Graham Pilling. 2020. {``Stock Assessment of
Yellowfin Tuna in the Western and Central Pacific Ocean.''}
\{WCPFC\}-\{SC\}16-2020/\{SA\}-\{WP\}-04-Rev2. WCPFC.

\bibitem[\citeproctext]{ref-vincent_incorporation_2018}
Vincent, M, G Pilling, and J Hampton. 2018. {``Incorporation of Updated
Growth Information Within the 2017 {WCPO} Bigeye Stock Assessment Grid,
and Examination of the Sensitivity of Estimates to Alternative Model
Spatial Structures.''} {WCPFC}-{SC}14-2018/ {SA}-{WP}-03. Oceanic
Fisheries Programme, The Pacific Community.

\bibitem[\citeproctext]{ref-wakefield_project_2026}
Wakefield, C., J. Widdrington, K. Krusic-Golub, A Pons, P. Reis-Santos,
B. Gillanders, J. Farley, S Nicol, and P. Hamer. 2026. {``Project 129
Update: Building an Age-Length Data Stream for Tuna Assessments.''}
{WCPFC}-{SC}22-2022/{SA}-{WP}-12. WCPFC.

\bibitem[\citeproctext]{ref-williams_overview_2023}
Williams, P., and T. Ruaia. 2023. {``Overview of Tuna Fisheries in the
