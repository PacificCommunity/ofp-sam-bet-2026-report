...
\caption{Recent spawning depletion relative to the 2012--2015 average across the 80 equal-weight central models, excluding estimation uncertainty. For each year, depletion is spawning biomass divided by mean unfished spawning biomass over the preceding ten years. The period values are arithmetic means across models; the final row is their ratio.}\label{tbl-cmm-depletion}\\
\toprule
Quantity & Value \\
\midrule
\endfirsthead
...
\caption{Stock status probabilities in relation to the adopted limit and MSY-based reference points.}\label{tbl-stock-status-refs}\\
\toprule
